...
export default function BugTrackerUI() {
    ...
    if (error) {
        return (
            <div className="flex-1 flex items-center justify-center bg-[#0a0a0a] text-blue-500 font-mono">
    ...
    return (
        <div className="flex flex-col w-full h-full bg-[#233657] text-neutral-300 font-sans border-l border-neutral-900 overflow-hidden relative">
            <div className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-[#152033] z-10 shrink-0">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <span className="text-blue-600">IT</span> Bug Tracker
                    </h2>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                {loading && bugs.length === 0 ? (
                    <div className="flex items-center justify-center h-40">
                        <span className="animate-pulse text-blue-500 font-bold tracking-widest text-sm">LEYENDO LOGS...</span>
                    </div>
                ) : bugs.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                        {bugs.map(bug => (
                            <div key={bug.id} 
                                ...
                                className={`flex flex-col bg-[#111] border rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 ${bug.resolved ? 'border-green-900/30 opacity-70' : 'border-blue-900/30 shadow-[0_4px_20px_rgba(0,153,204,0.1)] hover:-translate-y-1'}`}
                            >
                                
                                {bug.screenshot_url ? (
                                    <div className="h-40 bg-[#152033] relative border-b border-white/5 group">
                                        ...
                                        <a href={bug.screenshot_url} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="absolute top-3 right-3 bg-[#152033]/60 backdrop-blur-sm p-1.5 rounded-lg border border-white/10 hover:bg-white/20 transition opacity-0 group-hover:opacity-100">
                                ...
                                <div className="p-5 flex-1 flex flex-col">
                                    <div className="flex items-start justify-between mb-3 gap-2">
                                        <div className="flex gap-2 items-center flex-wrap">
                                            <span className={`text-[10px] uppercase font-black tracking-wider px-2 py-0.5 rounded-full border ${bug.priority === 'urgente' ? 'bg-blue-500/10 text-blue-500 border-blue-500/50' : bug.priority === 'baja' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30'}`}>
                                                {bug.priority}
                                            </span>
                                    ...
                                    <button 
                                        ...
                                        className={`w-full mt-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${bug.resolved ? 'bg-neutral-800 hover:bg-neutral-700 text-neutral-400' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-[0_0_15px_rgba(0,153,204,0.3)]'}`}
                                    >
            ...
            {selectedBug && (
                <div 
                    className="fixed inset-0 z-[100] bg-[#152033]/80 backdrop-blur-sm flex items-center justify-center p-4"
                ...
                >
                    ...
                    >
                        <button onClick={() => setSelectedBug(null)} className="absolute top-4 right-4 text-neutral-500 hover:text-white bg-[#152033]/50 rounded-full w-8 h-8 flex items-center justify-center z-10 transition">✕</button>
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {selectedBug.screenshot_url && (
                                <div className="w-full bg-[#152033] border-b border-white/5 relative group shrink-0">
                                    <img src={selectedBug.screenshot_url} alt="Evidencia" className="w-full max-h-[40vh] object-contain" />
                                    <a href={selectedBug.screenshot_url} target="_blank" rel="noreferrer" className="absolute bottom-4 right-4 bg-[#152033]/60 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/20 transition text-xs font-bold flex items-center gap-2 opacity-0 group-hover:opacity-100 text-white">
                                        Abrir Original
                            ...
                            <div className="p-8">
                                <div className="flex items-center gap-3 mb-6">
                                    <span className={`text-xs uppercase font-black tracking-wider px-3 py-1 rounded-full border ${selectedBug.priority === 'urgente' ? 'bg-blue-500/10 text-blue-500 border-blue-500/50' : selectedBug.priority === 'baja' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30'}`}>
                                ...
                                <div className="bg-[#152033]/50 border border-white/5 rounded-xl p-6 mb-8 text-white font-sans whitespace-pre-wrap leading-relaxed">
                        ...
                        <div className="p-4 border-t border-white/5 bg-[#0a0a0a] flex justify-end gap-3 rounded-b-2xl shrink-0">
                            ...
                            <button 
                                ...
                                className={`px-6 py-2 rounded-xl font-black text-sm uppercase tracking-widest transition-all ${selectedBug.resolved ? 'bg-neutral-800 hover:bg-neutral-700 text-neutral-400' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-[0_0_15px_rgba(0,153,204,0.3)]'}`}
                            >
