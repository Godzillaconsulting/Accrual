...
export default React.memo(function CockersStudio({ adminProfile, forceOpenEditor = false }) {
    ...
    const [savedCustomFilters, setSavedCustomFilters] = useState(() => {
        try { return JSON.parse(localStorage.getItem('accrual_custom_filters_v2') || '[]'); } catch { return []; }
    });
    ...
    const saveCustomFilter = () => {
        ...
        localStorage.setItem('accrual_custom_filters_v2', JSON.stringify(updated));
    ...
    const [customPresets, setCustomPresets] = useState(() => {
        try { return JSON.parse(localStorage.getItem('accrual_custom_presets') || '[]'); } catch { return []; }
    });
    ...
    const handleSavePreset = () => {
        ...
        localStorage.setItem('accrual_custom_presets', JSON.stringify(newPresets));
    };
    ...
    const TiltCard = ({ item, idx }) => {
        ...
        return (
            <motion.div
                ...
                className={`gallery-card relative group rounded-2xl overflow-hidden border border-white/5 hover:border-[#0099CC]/60 transition-colors shadow-xl shadow-black/60 aspect-video bg-[#0f0f0f] min-h-[220px] flex flex-col justify-center${isFeatured ? ' featured' : ''}`}
            >
                {/* Shimmer skeleton */}
                ...
                <div className="float-badge absolute top-3 right-3 z-30 pointer-events-none">
                    <span className="bg-[#152033]/70 backdrop-blur-xl border border-white/10 text-[9px] font-black uppercase tracking-widest text-neutral-300 px-2.5 py-1 rounded-full flex items-center gap-1.5">
                        <span className={`w-1.5 h-1.5 rounded-full ${isFeatured ? 'bg-[#0099CC] animate-pulse' : 'bg-emerald-400'}`}/>
                        {item.model}
                    </span>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 z-30 p-4 opacity-0 group-hover:opacity-100 translate-y-3 group-hover:translate-y-0 transition-all duration-300">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); toggleFilter(item.tag); }} className="flex-1 flex items-center justify-center gap-1 bg-white/10 hover:bg-[#0099CC]/50 backdrop-blur-xl border border-white/10 hover:border-[#0099CC]/50 rounded-full py-1.5 text-[9px] font-black text-white uppercase tracking-wider transition-colors cursor-pointer">
    ...
    return (
        <div className="flex w-full h-full bg-[#0a0a09] text-white overflow-hidden relative">
            ...
            <div className="w-[380px] bg-[#0f0f0e] border-r border-[#222] flex flex-col shrink-0 h-full overflow-y-auto custom-scrollbar z-20">
                ...
                <div className="p-4 pt-6 shrink-0 flex items-center justify-center">
                    <div className="bg-[#1a1a19] px-4 py-2 rounded-full flex items-center justify-center w-full shadow-inner border border-neutral-800/50">
                        <span className="text-[11px] font-black uppercase tracking-widest text-[#0099CC] flex items-center gap-2">
                ...
                <div className="px-4 pb-2 shrink-0">
                    <div className="flex bg-[#1a1a19] rounded-2xl p-1 border border-neutral-800 shadow-inner">
                        ...
                        <button
                            ...
                            className={`flex-1 py-2.5 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-1.5 ${genMode === 'video' ? 'bg-[#0099CC] text-white shadow-[0_0_15px_rgba(0,153,204,0.4)]' : 'text-neutral-500 hover:text-[#0099CC]'}`}
                        >
                ...
                <div className="px-5 py-2 flex flex-col flex-1 shrink-0">
                    ...
                    <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 mb-2 mt-2">
                        {dynamicFilters.slice(0, 10).map((preset, idx) => (
                            <button 
                                ...
                                className="shrink-0 bg-[#1a1a19] hover:bg-[#0099CC]/20 border border-neutral-800 hover:border-[#0099CC]/50 text-neutral-400 hover:text-white px-3 py-1.5 rounded-full text-[10px] font-bold uppercase transition-all flex items-center gap-1.5"
                            >
                                {preset.label}
                            </button>
                        ))}
                    ...
                    <div className="bg-[#141413] border border-neutral-800 rounded-3xl p-4 mt-4 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="flex bg-[#222221] rounded-full p-1 mb-4">
                            ...
                            <button onClick={()=>setActiveTab('Purificador')} className={`flex-1 py-1.5 text-[10px] uppercase tracking-widest font-bold rounded-full transition-colors ${activeTab==='Purificador' ? 'bg-[#0099CC] text-white shadow-[0_0_10px_rgba(0,153,204,0.5)]' : 'text-neutral-400 hover:text-[#0099CC]'}`}>
                                ✨ Limpiador
                            </button>
                        </div>
                        ...
                        {activeTab === 'Fotogramas' ? (
                                ...
                                <div className="mt-2 pt-2 border-t border-white/5">
                                    <p className="text-[9px] font-black uppercase tracking-widest text-neutral-500 mb-2 flex items-center justify-between">
                                        ...
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            <button onClick={() => setSelectedFilters([])} className="text-[7px] text-neutral-600 hover:text-blue-400 transition-colors">Limpiar</button>
                                        </span>
                                    </p>
                                        ...
                                        {BASE_PHOTO_FILTERS.map(filter => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    title={filter.prompt}
                                                    className={`px-2 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-full transition-all border flex items-center gap-1.5 ${isActive ? 'bg-[#0099CC] text-white border-[#0099CC] shadow-[0_0_10px_rgba(0,153,204,0.5)]' : 'bg-transparent text-neutral-500 border-neutral-800 hover:border-neutral-600 hover:text-white'}`}
                                                >
                                                    {filter.label}
                                                    {isActive && <span className="text-[7px] bg-blue-900/40 text-white rounded-full w-3.5 h-3.5 flex items-center justify-center">✕</span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                                                    ...
                                                    return (
                                                        <div key={filter.id} className="flex items-center gap-0.5">
                                                            ...
                                                            <button
                                                                ...
                                                                className="px-1 py-1.5 text-[8px] text-neutral-700 hover:text-blue-400 border-y border-r border-neutral-800 hover:border-blue-500/30 rounded-r-full transition-colors"
                                                            >🗑</button>
                                                        </div>
                                                    );
                                                })}
                            ...
                            <div className="flex flex-col gap-2 mb-4">
                                {refImage ? (
                                    <div className="relative group rounded-2xl overflow-hidden border-2 border-[#0099CC] shadow-[0_0_15px_rgba(0,153,204,0.3)]">
                                        <img src={refImage} alt="Ingrediente" className="w-full h-[120px] object-cover" />
                                        <div className="absolute inset-0 bg-[#152033]/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                                            <span className="text-white text-[10px] font-bold uppercase tracking-widest text-center px-2">Ingrediente <br/>Digerido</span>
                                            <button onClick={() => setRefImage('')} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-1 px-3 rounded-full text-[9px] uppercase">Quitar/Cambiar</button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex items-center flex-col justify-center h-[120px] border-2 border-dashed border-neutral-800 rounded-2xl hover:border-[#0099CC] hover:bg-[#0099CC]/10 transition-colors cursor-pointer relative">
                            ...
                            <div className="flex flex-col mb-4 p-4 border border-[#0099CC]/30 bg-[#0099CC]/5 rounded-2xl">
                                ...
                                {purifyingStatus ? (
                                    <div className="flex items-center justify-center p-6 border border-neutral-800 rounded-xl bg-[#152033]">
                                       <div className="flex flex-col items-center gap-2 animate-pulse">
                                          <div className="w-5 h-5 rounded-full border-t-2 border-[#0099CC] animate-spin"></div>
                                    ...
                                    <div className="flex items-center flex-col justify-center h-[100px] border-2 border-dashed border-[#0099CC]/50 rounded-xl hover:border-[#0099CC] hover:bg-[#0099CC]/10 transition-colors cursor-pointer relative">
                                        <span className="text-2xl mb-1">✨</span>
                        ...
                        <div className="mt-4 pt-4 border-t border-white/5">
                            <label className="text-[10px] font-black text-[#0099CC] uppercase mb-2 flex items-center gap-2"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M21.582,6.186c-0.23-0.86-0.908-1.538-1.768-1.768C18.254,4,12,4,12,4S5.746,4,4.186,4.418c-0.86,0.23-1.538,0.908-1.768,1.768C2,7.746,2,12,2,12s0,4.254,0.418,5.814c0.23,0.86,0.908,1.538,1.768,1.768C5.746,20,12,20,12,20s6.254,0,7.814-0.418c0.86-0.23,1.538-0.908,1.768-1.768C22,16.254,22,12,22,12S22,7.746,21.582,6.186z M9.996,15.005V8.995L15.266,12L9.996,15.005z"/></svg> Vincular YouTube (Final)</label>
                ...
                <div className="p-4 border-t border-[#222] bg-[#0a0a09] shrink-0">
                    ...
                    >
                        ...
                        {selectedDraft?.media_options?.length > 0 && !renderingAI && (
                            <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-[#0099CC]/10 backdrop-blur-md p-2 text-[9px] text-[#0099CC] border border-[#0099CC]/50 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-lg">
                    ...
                    <div className="flex gap-2">
                        {renderingAI && (
                            <button 
                                ...
                                className="flex-1 bg-blue-600/20 hover:bg-blue-600/40 text-blue-500 border border-blue-500/50 font-black uppercase tracking-widest text-[10px] py-2.5 rounded-full flex justify-center items-center gap-1.5 transition-colors"
                            >
                    ...
                    {refImage && (
                        <div className="mt-4 flex items-center justify-between bg-neutral-900 border border-neutral-800 rounded-xl p-2 px-3">
                            ...
                            <button onClick={()=>setRefImage('')} className="text-neutral-500 hover:text-[#0099CC] text-lg font-black ml-2 mb-1">×</button>
                        </div>
                    )}
                    </div>
                </div>
                ...
                {showScriptGen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#152033]/80 backdrop-blur-sm">
                        <div className="w-full max-w-2xl h-[70vh] bg-[#111111] border border-[#0099CC]/30 shadow-[0_0_50px_rgba(0,153,204,0.2)] rounded-3xl overflow-hidden flex flex-col relative transform scale-100 transition-all">
                            <div className="bg-[#0099CC]/10 border-b border-blue-900/30 p-4 shrink-0 flex justify-between items-center relative">
                                <div>
                                    <h3 className="text-[#0099CC] font-black uppercase text-sm tracking-widest flex items-center gap-2">✨ Redactor IA</h3>
                                    <p className="text-[10px] text-neutral-400 font-bold mt-1 uppercase">Conversa y obtén el copy definitivo</p>
                                </div>
                                <button onClick={() => setShowScriptGen(false)} className="text-white hover:text-[#0099CC] font-black text-xl w-8 h-8 flex items-center justify-center bg-[#152033]/30 backdrop-blur-md shadow-md hover:bg-[#0099CC]/20 rounded-full outline-none">×</button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-[#0a0a0a] to-[#010101]">
                                {scriptChatHistory.map((msg, i) => (
                                    <div key={i} className={`flex ${msg.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
                                        <div className={`p-4 rounded-xl max-w-[85%] text-sm font-bold shadow-lg leading-relaxed ${msg.role === 'ai' ? 'bg-[#1a1a1a] text-neutral-300 border border-neutral-800 rounded-tl-sm' : 'bg-[#0099CC]/20 border border-[#0099CC]/40 text-white rounded-tr-sm'}`}>
                                            {msg.text}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="p-4 bg-[#152033]/60 backdrop-blur border-t border-blue-900/30 flex gap-2 shrink-0">
                                <input 
                                    type="text" 
                                    ...
                                    className="flex-1 bg-[#152033]/50 backdrop-blur-md border hover:border-[#0099CC]/50 shadow-inner text-white focus:bg-[#0099CC]/10 rounded-xl p-3 text-sm focus:outline-none focus:border-[#0099CC] border-blue-900/30" 
                                />
                                <button 
                                    onClick={() => handleSendChatMessage(scriptChatInput)} 
                                    className="bg-[#222] hover:bg-gradient-to-r from-[#0099CC] to-[#006699] text-white font-black uppercase px-6 rounded-xl text-xs transition-colors shadow-lg"
                                >
                                    Enviar
                                </button>
                            </div>
            ...
            <div className="flex-1 bg-[#152033] relative flex flex-col overflow-hidden">
                ...
                <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-10 bg-gradient-to-b from-[#000000cc] to-transparent">
                    ...
                    <div className="flex gap-3">
                        ...
                        {canSeeAll && (
                            <button onClick={() => setShowOutbox(true)} className="text-xs font-bold text-[#0099CC] hover:bg-[#0099CC] hover:text-white border border-[#0099CC]/50 px-4 py-2 rounded-full transition-colors flex items-center gap-2 bg-[#0099CC]/10 shadow-[0_0_10px_rgba(0,153,204,0.2)]">
                            ...
                            <div className="bg-[#1a1a1a] border-b border-neutral-800 p-5 flex justify-between items-center">
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-3">
                                        <button onClick={() => setOutboxTab('enviadas')} className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full transition-all border ${outboxTab === 'enviadas' ? 'bg-[#0099CC] text-white border-[#0099CC]' : 'bg-transparent text-neutral-500 border-neutral-700 hover:text-white'}`}>Enviadas</button>
                                        ...
                                        <button onClick={() => setOutboxTab('correcciones')} className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full transition-all border ${outboxTab === 'correcciones' ? 'bg-[#0099CC] text-white border-[#0099CC]' : 'bg-transparent text-neutral-500 border-neutral-700 hover:text-white'}`}>Correcciones</button>
                                    </div>
                                </div>
                                <button onClick={() => setShowOutbox(false)} className="text-white hover:text-[#0099CC] text-xl font-black w-8 h-8 self-start">×</button>
                            </div>
                            
                            <div className="flex-1 overflow-y-auto p-6 bg-[#233657]">
                                {outboxQueue.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {outboxQueue.map(task => (
                                            <div key={task.id} className={`border rounded-xl p-4 flex flex-col gap-3 relative overflow-hidden ${task.status === 'rejected' ? 'bg-[#0099CC]/5 border-[#0099CC]/40' : task.status === 'approved' ? 'bg-yellow-900/10 border-yellow-500/40' : 'bg-neutral-900 border-neutral-800'}`}>
                                                <div className="flex items-center gap-2 mb-2">
                                                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${task.status === 'rejected' ? 'bg-[#0099CC] text-white' : task.status === 'approved' ? 'bg-yellow-600 text-white' : 'bg-yellow-500/20 text-yellow-500'}`}>
                                                ...
                                                {task.media_options && task.media_options[0] && (
                                                    task.media_options[0].url.includes('.mp4') || task.media_options[0].url.includes('.webm') ? (
                                                        <div className="relative group/vid">
                                                            ...
                                                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/vid:opacity-100 transition-opacity bg-[#152033]/40 backdrop-blur-sm rounded-lg">
                                                                <button 
                                                ...
                                                <div className="flex justify-end gap-2 mt-auto pt-2 border-t border-neutral-800/50">
                                                    ...
                                                    {task.status === 'rejected' && (
                                                        <button 
                                                            ...
                                                            className="text-[10px] bg-[#0099CC] hover:bg-blue-800 text-white font-bold px-4 py-1.5 rounded shadow-[0_0_10px_rgba(0,153,204,0.3)]"
                                                        >
                                                            Cargar al Canvas
                                                        </button>
                                                    )}
                    ...
                    >
                        ...
                        <div className="max-w-5xl mx-auto">
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <p className="text-[9px] font-black text-[#0099CC] uppercase tracking-[0.3em]">Generaciones en Vivo</p>
                                ...
                                {liveSlots.map((slot, i) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            className="rounded-2xl overflow-hidden border border-white/5 hover:border-[#0099CC]/40 transition-colors shadow-xl shadow-black/60 bg-[#0a0a0a]"
                                        >
                                            {/* Media Area */}
                                            <div className={`${arStyle} relative w-full overflow-hidden bg-neutral-900 flex`}>
                                                {slot.status === 'loading' ? (
                                                    ...
                                                    <div className="flex-1 flex flex-col items-center justify-center bg-gradient-to-br from-neutral-900 to-black gap-3 relative overflow-hidden">
                                                        ...
                                                        <div className="w-7 h-7 border-2 border-neutral-700 border-t-[#0099CC] rounded-full animate-spin" />
                                                        ...
                                                        <div className="w-2/3 h-1 bg-neutral-800 rounded-full overflow-hidden mt-1">
                                                            <div
                                                                className="h-full bg-gradient-to-r from-[#0099CC] to-blue-400 rounded-full transition-all duration-500"
                                                    ...
                                                    <div className="flex-1 flex flex-col items-center justify-center gap-2 bg-neutral-900 border-2 border-dashed border-blue-900/40 rounded-xl m-2 opacity-80 cursor-pointer p-4 text-center" onClick={() => simulateAIGeneration()}>
                                                        <span className="text-xl">⚠️</span>
                                                        <span className="text-[9px] text-[#0099CC] font-black uppercase tracking-widest leading-loose">
                                                            {slot.errorMsg || `Falla de motor ${slot.provider}`}
                                                        </span>
                                                        <span className="text-[8px] text-blue-500/80 bg-blue-900/10 px-2 py-1 rounded">Clic para Reintentar</span>
                                                    </div>
                                                ) : slot.isVideo ? (
                                                    /* Video Done */
                                                    ...
                                                    <>
                                                        <div className="flex-1 relative cursor-pointer group/orig" onClick={() => handleMediaClick(slot.url)}>
                                                            ...
                                                            <div className="absolute top-2 left-2 bg-[#152033]/60 px-2 py-0.5 rounded text-[7px] uppercase tracking-wider text-white backdrop-blur-sm pointer-events-none">Original</div>
                                                        </div>
                                                        {/* Gemini Ultra Pane */}
                                                            ...
                                                            {slot.refinedUrl === 'loading' ? (
                                                                ...
                                                                <div className="flex flex-col items-center justify-center h-full gap-2 p-2">
                                                                    {slot.refinedUrl === 'error' && (
                                                                        <span className="text-[8px] text-center text-blue-500 font-bold mb-1 leading-tight">{slot.refinedError || 'Error al conectar'}</span>
                                                                    )}
                                                                    <button
                                                    ...
                                                    <div className="flex items-center gap-1.5 shrink-0">
                                                        ...
                                                        {isCockers && (
                                                            <button
                                                                ...
                                                                className="flex items-center gap-1 bg-[#0099CC]/15 hover:bg-[#0099CC] border border-[#0099CC]/40 hover:border-[#0099CC] text-[#0099CC] hover:text-white text-[8px] font-black uppercase tracking-wider px-2.5 py-1.5 rounded-full transition-all"
                                                            >
                ...
                {!renderingAI && liveSlots.length === 0 && (
                    ...
                    >
                        ...
                        <div className="pointer-events-none absolute inset-0 overflow-hidden z-0">
                            <div className="absolute -top-32 -left-32 w-[500px] h-[500px] bg-[#0099CC]/8 rounded-full blur-[120px]" />
                        ...
                        <div className="sticky top-0 z-40 bg-gradient-to-b from-[#0a0a09] via-[#0a0a09]/95 to-transparent pt-6 pb-6 px-8">
                            <div className="flex items-center justify-between max-w-5xl mx-auto">
                                <div>
                                    <p className="text-[9px] font-black text-[#0099CC] uppercase tracking-[0.3em] mb-0.5">
                                        Accrual Studio AI — Motores Flash
                                    </p>
                                    <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
                                        Director's Gallery
                                        <span className="text-[9px] font-black bg-[#0099CC]/20 text-[#0099CC] border border-[#0099CC]/30 px-2 py-0.5 rounded-full uppercase tracking-wider">
                                            LIVE
                                        </span>
                                    </h1>
                                </div>
                        ...
                        <div className="overflow-hidden border-y border-white/5 bg-[#152033]/30 backdrop-blur-sm py-2 mb-6">
                    ...
                    >
                        <div className="max-w-5xl mx-auto">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button 
                                    onClick={() => simulateAIGeneration()}
                                    className="bg-[#0099CC]/20 hover:bg-[#0099CC]/40 text-[#0099CC] border border-[#0099CC]/50 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center gap-2 shadow-lg hover:shadow-[#0099CC]/20"
                                ...
                                {selectedDraft.media_options.map((opt, i) => (
                                    <motion.div 
                                        ...
                                        <div className="absolute top-5 left-5 z-20 flex gap-2">
                                            <span className="bg-[#152033]/80 backdrop-blur-md px-3 py-1 rounded-full shadow text-white font-bold text-[10px] tracking-wider uppercase border border-white/10 group-hover:border-white/30 transition-colors">
                                                {opt.isVideo && (opt.url.includes('.mp4') || opt.url.includes('.webm')) ? 'VIDEO' : opt.isVideo ? 'CONCEPT FRAME' : 'IMAGE'}
                                            </span>
                                            <span className="bg-[#152033]/80 backdrop-blur-md px-3 py-1 rounded-full shadow text-neutral-400 font-bold text-[10px] uppercase border border-white/10">
                                                {builderData.aspect_ratio}
                                            </span>
                                        </div>
                                        
                                        <div className={`w-full aspect-video bg-[#111] rounded-2xl overflow-hidden relative flex gap-1 ${opt.isVideo || opt.provider.includes('GotSora') ? '' : 'p-1 bg-[#152033]/40 border border-neutral-800'}`}>
                                            {opt.isVideo ? (
                                                <div className="w-full h-full relative group/vid overflow-hidden bg-[#152033] flex items-center justify-center cursor-pointer" onClick={() => handleMediaClick(opt.url)}>
                                                    {getYouTubeId(opt.url) ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <div style={{display:'none'}} className="absolute inset-0 flex-col items-center justify-center bg-neutral-900 gap-3">
                                                                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0099CC" strokeWidth="1.5"><path d="m22 8-6 4 6 4V8Z"/><rect width="14" height="12" x="2" y="6" rx="2"/><line x1="2" y1="2" x2="22" y2="22" stroke="#666"/></svg>
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex-1 rounded-xl overflow-hidden relative cursor-pointer group/orig" onClick={() => handleMediaClick(opt.url)}>
                                                        ...
                                                        {!opt.provider.includes('GotSora') && <div className="absolute top-2 left-2 bg-[#152033]/60 px-2 py-0.5 rounded text-[8px] uppercase tracking-wider text-white backdrop-blur-sm pointer-events-none shadow-md">Original</div>}
                                                    </div>
                                                ...
                                                {canSeeAll && (
                                                    <div className="flex gap-1.5">
                                                        {adminProfile?.username?.toLowerCase() === 'alex' && (
                                                            <button onClick={() => handleAction(opt, 'review')} className="bg-[#0099CC] hover:bg-blue-800 text-white font-bold text-[9px] uppercase tracking-wider px-4 py-2 rounded-full shadow-[0_0_10px_rgba(0,153,204,0.4)] transition-transform active:scale-95">
                            ...
                            <div className="mt-8 bg-gradient-to-b from-[#111] to-[#0a0a0a] border border-[#0099CC]/20 rounded-3xl p-5 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                                ...
                                <div className="flex flex-col md:flex-row gap-3">
                                    <input 
                                        ...
                                        className="flex-1 bg-[#161616] border border-neutral-800 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-[#0099CC] text-sm transition-colors shadow-inner"
                                    />
                                    <button 
                                        onClick={() => simulateAIGeneration()}
                                        className="bg-[#0099CC] hover:bg-blue-800 text-white font-black px-8 py-4 md:py-0 rounded-xl text-xs uppercase tracking-widest transition-transform active:scale-95 flex justify-center items-center gap-3 shadow-[0_0_15px_rgba(0,153,204,0.4)]"
                                    >
                ...
                <div className="flex-1 min-h-[400px] border-t-2 border-[#0099CC]/50 bg-[#0a0a09] flex flex-col z-40 relative shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
            ...
            {showCustomFilterModal && (
                <div className="fixed inset-0 z-[100] bg-[#152033]/90 backdrop-blur-sm flex items-center justify-center p-4">
