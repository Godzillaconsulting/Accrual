...
export default function NewsletterPanel() {
    ...
    const handleDeleteSubscriber = async (id, email) => {
        if (!window.confirm(`¿Estás seguro de borrar permanentemente a ${email} de la base de datos de Accrual?`)) return;
        try {
    ...
    useEffect(() => {
        ...
        if (tab === 'socios') {
            const node = nodes.find(n => n.id === 'socio-accrual');
            let data = node ? { ...(node.published_data || {}), ...(node.draft_data || {}) } : {};
            data = injectSectionDefaults('socio-accrual', data);
            setSociosDraft(data);
    ...
    useEffect(() => {
        if (tab === 'socios' && sociosDraft) {
            setPreviewOverride('socio-accrual', sociosDraft);
        }
    }, [sociosDraft, tab]);
    ...
    const handleSaveSocios = async () => {
        ...
        try {
            const r = await fetch(`${API_BASE}/api/nodes/socio-accrual/draft`, {
                method: 'PUT',
                headers: authHeaders(),
            ...
            const p = await fetch(`${API_BASE}/api/nodes/socio-accrual/publish`, {
                method: 'POST',
                headers: authHeaders(),
                body: JSON.stringify({}),
            });
            ...
            setSociosHasChanges(false);
            alert("✅ Cambios de Socio Accrual guardados y publicados.");
        } catch (err) {
            alert(err.message);
        }
    ...
    const statusBadge = (status) => {
        const map = {
            ...
            failed: { color: 'text-blue-400 bg-blue-400/10', icon: <XCircle size={12} />, label: 'Falló' },
    ...
    return (
        <div className="flex flex-col h-full text-white">
            ...
            <div className="px-6 py-4 border-b border-neutral-800 shrink-0">
                ...
                <p className="text-[10px] text-neutral-500 mt-0.5">Genera investigación dual (Freemium + Socios) y corrige iterativamente con Accrual. </p>
                
                {currentDraftId && (
                    <div className="mt-3 inline-flex items-center gap-2 bg-[#0099CC]/10 text-[#0099CC] border border-[#0099CC]/20 px-3 py-1.5 rounded-full text-[10px] font-bold">
                ...
                {[
                    ...
                    <button key={t.id} onClick={() => setTab(t.id)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                            tab === t.id ? 'bg-[#0099CC] text-white' : 'bg-neutral-800 text-neutral-400 hover:text-white'
                        }`}>
                        {t.label}
                    </button>
                ))}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-xl space-y-3 mb-4">
                            ...
                            <input
                                ...
                                className="w-full p-2.5 bg-[#152033] border border-neutral-700 rounded-lg text-white text-xs focus:border-yellow-500 outline-none"
                            />
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                placeholder="https://accrual.ai/media/guia.pdf"
                                className="w-full p-3 bg-[#152033] border border-neutral-700 rounded-xl text-white text-sm focus:border-[#0099CC] outline-none"
                                readOnly
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                className="w-full p-3 bg-[#152033] border border-neutral-700 rounded-xl text-white text-sm font-mono focus:border-[#0099CC] outline-none resize-none"
                            />
                        </div>
                        ...
                        <button
                            ...
                            className={`w-full flex items-center justify-center gap-2 text-white py-3 rounded-xl font-black text-sm transition shadow-[0_4px_20px_rgba(0,153,204,0.4)] disabled:opacity-40 disabled:cursor-not-allowed ${
                                successPulse ? 'bg-green-600 hover:bg-green-500' : 'bg-[#0099CC] hover:bg-blue-600'
                            }`}
                        >
                            {sending
                                    ...
                                    <div key={s.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="text-neutral-500 hover:text-blue-500 hover:bg-blue-500/10 transition px-2 py-1 rounded-md bg-[#152033] border border-neutral-800" 
                                ...
                                {history.map(n => {
                                    ...
                                    <div key={n.id} className="px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-800 space-y-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {n.status === 'draft' && (
                                                    <button onClick={() => {
                                                        ...
                                                        setTab('compose');
                                                    }} className="text-[#0099CC] bg-white/5 hover:bg-[#0099CC] hover:text-white px-2 py-1 rounded-full text-[10px] font-bold transition-all flex items-center gap-1">
                                                        ✏️ Edición Manual
                                                    </button>
                                                )}
                                                <button onClick={() => handleDelete(n.id)} className="text-neutral-500 hover:text-blue-500 hover:bg-neutral-800 px-2 py-1 rounded-full text-[10px] font-bold transition-all flex items-center gap-1" title="Borrar Reporte">
                                                     🗑️
                                                </button>
                                        ...
                                        <div className="flex items-center gap-4 text-[10px] text-neutral-500">
                                            ...
                                            {n.failed_count > 0 && <span className="flex items-center gap-1"><XCircle size={10} className="text-blue-400" /> {n.failed_count} falló</span>}
                ...
                {tab === 'socios' && (
                    <div className="space-y-4 flex flex-col h-full overflow-hidden">
                        ...
                        <div className="flex flex-col lg:flex-row gap-4 flex-1 h-full min-h-[500px]">
                            ...
                            <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                                {Object.entries(sociosDraft).map(([key, val]) => (
                                    <div key={key} className="space-y-1.5">
                                        ...
                                        {val && val.length > 60 ? (
                                             <textarea rows={3} value={val} onChange={e => { setSociosDraft({...sociosDraft, [key]: e.target.value}); setSociosHasChanges(true); }} className="w-full p-2.5 bg-[#152033] border border-neutral-700 rounded-lg text-white text-xs focus:border-yellow-500 outline-none resize-none" />
                                        ) : (
                                             <input type="text" value={val} onChange={e => { setSociosDraft({...sociosDraft, [key]: e.target.value}); setSociosHasChanges(true); }} className="w-full p-2.5 bg-[#152033] border border-neutral-700 rounded-lg text-white text-xs focus:border-yellow-500 outline-none" />
                                        )}
                                    </div>
                                ))}
                            </div>
                            ...
                            {/* Live Preview */}
                            <div className="flex-1 bg-[#152033] rounded-xl border border-neutral-800 flex flex-col">
