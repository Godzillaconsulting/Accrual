...
const CanvasCaptcha = forwardRef(({ onValidate, height = 50, length = 5 }, ref) => {
    ...
    const drawCaptcha = (text) => {
        ...
        // Add noise lines
        for (let i = 0; i < 5; i++) {
            ctx.strokeStyle = `rgba(0, 188, 212, ${Math.random() * 0.5})`;
            ctx.beginPath();
        ...
        // Add noise dots
        for (let i = 0; i < 30; i++) {
            ctx.fillStyle = `rgba(24, 255, 255, ${Math.random() * 0.5})`;
            ctx.beginPath();
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            ctx.rotate(angle);
            // Cyan/Teal Accrual colors for text
            ctx.fillStyle = `hsl(${180 + Math.random() * 40}, 80%, 60%)`;
    ...
    return (
        <div className="flex flex-col gap-3 w-full">
            <div className="flex items-center gap-3">
                <div className="relative flex-1 bg-black/40 rounded-xl border border-white/10 overflow-hidden flex items-center justify-center p-1 cursor-pointer" onClick={generateCaptcha} title="Clic para generar otro">
                    <canvas 
                        ref={canvasRef} 
                        width={150} 
                        height={height}
                ...
                <button 
                    ...
                    className="w-11 h-11 shrink-0 bg-black/40 border border-white/10 hover:border-[#00bcd4]/50 hover:bg-[#00bcd4]/10 rounded-xl flex items-center justify-center text-white/50 hover:text-[#00bcd4] transition-all shadow-inner"
                >
                    <RefreshCw size={18} />
                </button>
            </div>
            ...
            {isValid === false && inputValue.length > 0 && (
                <p className="text-red-400 text-[10px] font-black uppercase tracking-widest px-1">Validación visual incorrecta.</p>
            )}
        </div>
    );
});
