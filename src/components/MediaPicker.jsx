...
export default function MediaPicker({ value, onChange, accept = 'all', label = 'Imagen / Media' }) {
    ...
    return (
        <div className="space-y-2">
            ...
            <div
                className="relative shrink-0 w-32 h-32 bg-neutral-800 border border-neutral-700 rounded-xl overflow-hidden cursor-pointer group hover:border-[#0099CC] p-2 flex items-center justify-center transition-colors"
            ...
            >
                ...
                {/* Overlay */}
                <div className="absolute inset-0 bg-[#152033]/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="text-white text-xs font-bold bg-[#152033]/60 px-3 py-1.5 rounded-full">
                        ✏️ Cambiar archivo
                    </span>
                </div>
            </div>
            ...
            {/* Modal */}
            {isOpen && createPortal(
                <div className="fixed inset-0 z-[100] bg-[#152033]/80 backdrop-blur-sm flex items-center justify-center p-6" style={{ zIndex: 99999 }}>
                                            ...
                                            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                                {filteredItems().map(item => (
                                                    <div
                                                        ...
                                                        className={`relative group cursor-pointer rounded-2xl overflow-hidden border-[3px] transition-all hover:scale-[1.02] ${value === item.url ? 'border-[#0099CC] shadow-[0_0_20px_rgba(0,153,204,0.4)]' : 'border-neutral-800 hover:border-neutral-500'}`}
                                                    >
                                                        <div className="aspect-square bg-[#152033] flex items-center justify-center relative overflow-hidden">
                                                            {item.type === 'document' ? (
                                                        ...
                                                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
                                                            ...
                                                            <div className="flex items-center justify-between mt-1">
                                                                ...
                                                                <span className="text-[#0099CC] text-[10px] font-bold">Seleccionar</span>
                                                            </div>
                                                        </div>
                                                        {/* Botón eliminar */}
                                                        <button
                                                            onClick={(e) => handleDelete(item.type, item.filename, e, item.url)}
                                                            className="absolute top-2 right-2 bg-blue-600/80 backdrop-blur-md text-white text-[12px] w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-blue-500 hover:scale-110"
                                                        ...
                                                        {/* Marker seleccionado */}
                                                        {value === item.url && (
                                                            <div className="absolute top-2 left-2 bg-[#0099CC] text-white text-[10px] px-2 py-1 rounded-full font-black shadow-lg">
                                                                En uso
                                                            </div>
                                                        )}
                                                    </div>
                                    ...
                                    {uploading ? (
                                        <div className="w-full max-w-xs">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <span className="text-sm text-[#0099CC] font-bold">{uploadProgress}%</span>
                                            </div>
                                        ...
                                        <>
                                            <div
                                                ...
                                                className="w-full max-w-sm border-2 border-dashed border-neutral-600 hover:border-[#0099CC] hover:bg-[#0099CC]/10 rounded-2xl p-10 cursor-pointer transition-all text-center group"
                                            >
                            ...
                            {tab === 'url' && (
                                <div className="flex flex-col gap-4 max-w-lg mx-auto mt-8">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-3 bg-[#152033] border border-neutral-700 rounded-xl text-white focus:border-[#0099CC] outline-none text-sm"
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-3 bg-[#0099CC] text-white rounded-full font-black disabled:opacity-40 hover:bg-blue-600 transition"
                                    >
                                        ✅ Usar esta URL
                                    </button>
                                </div>
