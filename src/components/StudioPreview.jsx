...
function LandingCardPreview({ draftData }) {
 ...
 {/* Details card */}
 <div className="max-w-2xl mx-auto bg-[#1c1c1c] border border-[#0099CC] rounded-3xl p-8 shadow-2xl">
 ...
 <div className="w-full py-4 bg-[#0099CC] text-white rounded-full font-black text-lg">Contáctanos</div>
 </div>
 </div>
 </div>
 );
...
const LANDING_HIGHLIGHT = {'heroTitle': ['h1'],'heroTopText': ['p.text-lg'],'planPrice': ['[class*="text-5xl"]'],'guaranteeText': ['[class*="rounded-2xl"]'],'imageUrl': ['video, img'],'videoFileUrl': ['video'],'accentColor': ['h1','[class*="bg-\\[\\#0099CC\\]"]'],
};

const LANDING_IDS = new Set(['paquete-posicionamiento-social','paquete-expansion','paquete-control-ia','paquete-elite',
]);

// LandingPaqueteDynamic eliminado
...
const Packages = lazy(() => import('../components/Services'));
...
const COMPONENT_MAP = {
  'hero': Hero,
  'auditoria-cta': AuditCTA,
  'paquetes': Packages,
  'testimonios': Testimonials,
  'quienes-somos': About,
  'servicios-grid': ServicesOverview,
  'articulos-grid': Articles,
  // Los demás IDs por defecto dirán "Sin preview"
};
...
function DynamicPreview({ nodeId }) {
    if (nodeId.startsWith('servicio-')) {
        const slug = SLUG_MAP[nodeId] || 'consultoria';
        return <ServiceDetail slug={slug} />;
    }
    if (nodeId.startsWith('articulo-')) {
        const id = SLUG_MAP[nodeId] || '1';
...
function ScaledSection({ nodeId }) {
 ...
 const isDynamic = nodeId.startsWith('servicio-') || nodeId.startsWith('articulo-');
 ...
 <div className="flex items-center justify-center h-64 gap-3 text-neutral-500">
 <div className="w-4 h-4 border-2 border-[#0099CC] border-t-transparent rounded-full animate-spin" />
 Cargando vista dinámica...
 </div>
 }>
 <DynamicPreview nodeId={nodeId} />
 </Suspense>
 )
 : (
...
export default function StudioPreview({ nodeId, draftData, hoveredField }) {
 ...
 {hoveredField && (
 <span className="ml-2 text-[10px] bg-[#0099CC]/10 text-[#0099CC] border border-[#0099CC]/20 px-2 py-0.5 rounded-full font-bold">
 ✦ {hoveredField}
 </span>
 )}
