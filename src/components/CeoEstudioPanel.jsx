...
const STATUS_MAP = {
    ...
    rejected:            { label: '🔙 Devuelta',    tab: 'devueltas',  color: 'text-blue-400 bg-blue-500/10 border-blue-500/30' },
...
export default function CeoEstudioPanel({ adminProfile }) {
    ...
    const canReview  = isCockers || ['judith', 'accrual_admin'].includes(username) || adminProfile?.is_superadmin;
    const canPublish = isCockers || ['judith', 'accrual_admin'].includes(username) || adminProfile?.is_superadmin;
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            const BASE_URL = 'https://accrual.ai';
    ...
    return (
        <div className="flex-1 flex flex-col p-6 bg-[#152033] text-white overflow-hidden relative">
            {/* Ambient Orb */}
            ...
            <div className="flex gap-3 mb-6 shrink-0 z-10 relative">
                {[
                    ...
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        ...
                        {tab.icon} {tab.label}
                        {counts[tab.id] > 0 && (
                            <span className={`absolute -top-1.5 -right-1.5 w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-black text-white ${tab.id === 'pendientes' ? 'bg-[#0099CC]' : 'bg-[#d946ef]'}`}>
                                {counts[tab.id]}
                            </span>
                        )}
                    </button>
                ...
                {!loading && visible.map(item => {
                    ...
                    return (
                        <div key={item.id} onClick={() => { setSelected(item); setFeedback(''); setPublishReport(null); }}
                            ...
                            <div className="h-40 bg-[#152033] flex items-center justify-center overflow-hidden relative">
                                {media?.url ? (
                                ...
                                {canReview && (
                                    <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button
                                            ...
                                            className="w-7 h-7 rounded-full bg-blue-500/20 hover:bg-blue-500 border border-blue-500/40 text-blue-400 hover:text-white text-[11px] font-black flex items-center justify-center transition-all"
                                        >✕</button>
                                    </div>
                                )}
                            </div>
            ...
            {selected && !showPublish && (
                <div className="fixed inset-0 z-[100] bg-[#152033]/90 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-neutral-900 border border-neutral-700 w-full max-w-5xl h-[88vh] rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-[0_0_50px_rgba(217,70,239,0.15)]">
                        ...
                        {/* Visualizador */}
                        <div className="flex-1 bg-[#152033] flex items-center justify-center relative p-4 min-h-[50%]">
                            {firstMedia?.url ? (
                                isVideo ? (
                        ...
                        <div className="w-full md:w-96 bg-neutral-950 p-6 flex flex-col border-l border-neutral-800 overflow-y-auto">
                            <div className="mb-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {canReview && (
                                        <button onClick={(e) => handleDelete(selected.id, e)} className="text-[10px] font-bold text-blue-500 hover:text-white border border-blue-500/30 hover:bg-blue-500 px-3 py-1 rounded-full transition-colors flex items-center gap-1.5 focus:outline-none">
                                            🗑 Borrar Activo
                                        </button>
                                    )}
                                </div>
                                ...
                                <div className="flex-1 flex flex-col">
                                    ...
                                    <div className="mt-auto space-y-3">
                                        ...
                                        <button onClick={() => handleAction('reject')}
                                            className="w-full bg-transparent border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white font-black py-4 rounded-xl text-lg transition-all">
                            ...
                            {selected.status === 'rejected' && (
                                <div className="flex-1 flex flex-col gap-3">
                                    <div className="bg-blue-500/10 border border-blue-500/30 p-4 rounded-xl">
                                        <p className="text-xs font-bold text-blue-400 mb-2 uppercase">Notas de devolución:</p>
                                    ...
                                    {canReview && (
                                        <div className="flex gap-2 mt-4 pt-4 border-t border-neutral-800">
                                            ...
                                            <button 
                                                ...
                                                className="flex-1 border border-blue-500/40 text-blue-500 hover:bg-blue-500 hover:text-white py-2 rounded-xl text-xs font-bold transition-colors"
                                            >
                                                ✕ Eliminar Activo
                                            </button>
                                        </div>
            ...
            {/* ── HUD DE PUBLICACIÓN ── */}
            {showPublish && selected && (
                <div className="fixed inset-0 z-[200] bg-[#152033]/95 flex items-center justify-center p-4">
                    <div className="w-full max-w-4xl flex gap-6 h-[82vh]">
                        ...
                        <div className="flex-1 bg-neutral-900 border border-neutral-700 rounded-3xl p-6 flex flex-col">
                            <div className="flex items-center justify-between border-b border-neutral-800 pb-4 mb-4">
                                ...
                                <div className="flex items-center gap-4">
                                    {canReview && (
                                        <button onClick={(e) => handleDelete(selected.id, e)} className="text-[10px] font-bold text-blue-500 hover:text-white border border-blue-500/30 hover:bg-blue-500 px-3 py-1.5 rounded-full transition-colors flex items-center gap-1.5 focus:outline-none">
                                            🗑 Borrar
                                        </button>
                                    )}
                            ...
                            <div className="flex gap-2 mb-6">
                                ...
                                <button onClick={() => setNetwork('tiktok')}    className={`flex-1 py-2 rounded-xl text-sm font-bold border ${network === 'tiktok'    ? 'bg-[#152033] border-[#00f2fe] text-white shadow-[0_0_10px_rgba(0,242,254,0.3)]' : 'bg-neutral-800 border-neutral-700 text-neutral-400'}`}>TikTok</button>
                            </div>
                        ...
                        {/* Preview celular */}
                        <div className="w-[375px] bg-[#152033] border-[8px] border-neutral-800 rounded-[3rem] overflow-hidden flex flex-col relative shadow-[0_0_50px_rgba(255,255,255,0.08)]">
                            ...
                            {network === 'instagram' && (
                                <div className="flex-1 bg-white text-black flex flex-col pt-8">
                                    <div className="flex items-center justify-between p-3 border-b border-gray-200">
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 rounded-full p-[2px]">
                                                <div className="w-full h-full bg-white rounded-full flex items-center justify-center p-0.5"><img src="/favicon.png" className="rounded-full bg-[#152033]" alt="" /></div>
                                            </div>
                                            <p className="font-bold text-sm">accrual.consulting</p>
                                        </div>
                                        <span className="font-black">⋮</span>
                                    </div>
                                    <div className="bg-[#152033] w-full aspect-square flex items-center justify-center overflow-hidden">
                                    ...
                                    <div className="p-3">
                                        ...
                                        <p className="text-sm line-clamp-3"><span className="font-bold">accrual.consulting</span> {caption || 'Tu texto de publicación aquí...'}</p>
                                    </div>
                                </div>
                            )}
                            {network === 'facebook' && (
                                <div className="flex-1 bg-neutral-200 text-black flex flex-col pt-8">
                                    <div className="bg-white p-3 mb-2">
                                        <div className="flex items-center gap-2 mb-2">
                                            <img src="/favicon.png" className="w-10 h-10 rounded-full bg-[#152033]" alt="" />
                                            <div>
                                                <p className="font-bold text-sm leading-none">Accrual</p>
                                                <p className="text-xs text-gray-500">Justo ahora • 🌎</p>
                                            </div>
                                        </div>
                                        <p className="text-sm mb-2">{caption || 'Tu texto aquí...'}</p>
                                        {firstMedia?.url && <div className="bg-[#152033]"><img src={firstMedia.url} className="w-full object-cover" alt="" /></div>}
                                    </div>
                                </div>
                            )}
                            {network === 'tiktok' && (
                                <div className="flex-1 bg-[#152033] text-white flex flex-col relative pt-0">
                                    ...
                                    <div className="absolute inset-y-0 right-2 flex flex-col justify-end pb-20 gap-4">
                                        <div className="w-10 h-10 rounded-full bg-white border-2 border-white overflow-hidden"><img src="/favicon.png" className="bg-[#152033]" alt="" /></div>
