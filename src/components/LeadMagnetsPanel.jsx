                        ...
                        {magnets.map(m => (
                            <button 
                                ...
                                className={`w-full text-left px-3 py-2 rounded-lg transition-all ${
                                    ...
                                    ? 'bg-neutral-800 border-l-2 border-[#0099CC] text-white shadow-sm'
                        ...
                        <div className="max-w-2xl space-y-5">
                            ...
                            <div className="flex items-center justify-between border-b border-neutral-800 pb-4">
                                <h3 className="text-lg font-black text-white">
                                    ...
                                    <span className="text-[#0099CC] ml-2">[{formData.slug || 'slug-vacio'}]</span>
                                </h3>
                                
                                <button
                                    onClick={handleSave}
                                    disabled={saving || !formData.slug || !formData.email_subject || !formData.file_url || !(formData.email_body || '').trim()}
                                    className="px-5 py-2 flex items-center gap-2 bg-[#0099CC] hover:bg-blue-600 text-white rounded-lg font-black text-xs transition shadow-[0_4px_15px_rgba(0,153,204,0.3)] disabled:opacity-40"
                                >
                            ...
                            {saveStatus && (
                                <div className={`flex items-start gap-2 p-3 rounded-xl border text-sm ${
                                    saveStatus.success ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400'
                                }`}>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-2.5 bg-[#152033] border border-neutral-700 rounded-xl text-white text-sm focus:border-[#0099CC] outline-none"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <input 
                                        ...
                                        className={`w-full p-2.5 rounded-xl border text-sm outline-none transition-colors ${
                                            isCreating 
                                            ? 'bg-[#152033] border-yellow-700/50 text-yellow-100 focus:border-yellow-500' 
                            ...
                            <div className="space-y-1.5 bg-neutral-900 p-4 rounded-2xl border border-neutral-800">
                                ...
                                <textarea 
                                    ...
                                    className="w-full p-3 mt-2 bg-[#152033] border border-neutral-700 rounded-xl text-white text-sm focus:border-[#0099CC] outline-none resize-none"
                                    placeholder="Hola, muchas gracias por confiar en Accrual. A continuación encontrarás el material prometido..."
                                />
                            </div>

                            {/* Enlace de descarga */}
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-[#0099CC] uppercase flex items-center gap-1"><LinkIcon size={10}/> Link Archivo / PDF</label>
                                <input 
                                    type="url" 
                                    value={formData.file_url} 
                                    onChange={e => setFormData({...formData, file_url: e.target.value})}
                                    className="w-full p-2.5 bg-[#0099CC]/5 border border-[#0099CC]/30 rounded-xl text-blue-100 text-sm focus:border-[#0099CC] outline-none"
                                    placeholder="https://accrual.ai/lead-magnets/archivo.pdf"
                                />
