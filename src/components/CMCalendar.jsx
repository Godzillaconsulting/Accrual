...
const canAssign = (profile) => {
    ...
    return usr === 'accrual_admin' || usr === 'judith' || usr === 'jareg' || usr === 'oscar' || usr === 'alex' || profile.is_superadmin;
};
...
const renderMentions = (text) => {
    ...
    return parts.map((part, i) =>
        part.startsWith('@')
            ? <span key={i} className="text-[#0099CC] font-black">{part}</span>
...
const STATUS_META = {
    urgent:  { bg: 'bg-blue-500/20',    border: 'border-blue-500',    text: 'text-blue-400',    glow: '0 0 12px rgba(0,153,204,0.5)',       label: 'Urgente'    },
...
export default React.memo(function CMCalendar({ adminProfile }) {
    ...
    const [sheetsEmpresa, setSheetsEmpresa] = useState('accrual'); // empresa destino del import
    ...
    const handleCreateCampaign = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setNewCampaign({ empresa: 'accrual', calendario: 'contenido', asignado: '', titulo: '', fecha: '', plataforma: 'ALL', briefing: '', urlFoto: '', urlVideo: '', urlReferencia: '' });
    ...
    const myFilteredTasks = tasks.filter(t =>
        t.para?.toLowerCase() === currentUser.toLowerCase() || currentUser.toLowerCase() === 'accrual_admin'
    );
    ...
    const eventStyleGetter = (event) => {
        ...
        if (event.status === 'urgent') { backgroundColor = '#0099CC'; border = '1px solid #ff4444'; }
    ...
    const renderNeuronCard = (event) => {
        ...
        return (
            ...
            >
                {/* Thumbnail */}
                {event.media_url && (
                    <div className="relative h-16 bg-[#152033] overflow-hidden">
                        <img
                ...
                <div className="px-2 py-1.5">
                    ...
                    {canCreate && (
                        <button
                            ...
                            className="mt-1.5 w-full opacity-0 group-hover:opacity-100 transition-opacity text-[8px] font-black bg-[#0099CC]/80 hover:bg-[#0099CC] text-white py-1 rounded flex items-center justify-center gap-1"
                        >
                            ✨ Generar en Studio
                        </button>
                    )}
                        ...
                        return (
                            <div
                                ...
                                className={`
                                    ...
                                    ${currentMonth || calendarView === 'week' ? 'bg-transparent' : 'bg-[#152033]/20'}
                                ...
                                <div className="flex items-center justify-between shrink-0">
                                    ...
                                    {canCreate && currentMonth && (
                                        <button
                                            ...
                                            className="opacity-0 group-hover/day:opacity-100 w-4 h-4 rounded-full bg-white/10 hover:bg-[#0099CC] text-white text-[10px] flex items-center justify-center transition-all"
                                        >
                                            +
                                        </button>
                                    )}
    ...
    const renderSocialMockup = (event) => (
        <div className="bg-white rounded-xl overflow-hidden shadow-xl text-black font-sans border border-white/20">
            {event.platform === 'instagram' && (
                <div>
                    <div className="flex items-center gap-3 p-3 border-b border-gray-100">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-400 via-blue-500 to-purple-500 p-[2px]">
                            <div className="w-full h-full rounded-full bg-white border-2 border-white overflow-hidden"><img src="/logo192.png" alt="Accrual" className="w-full h-full object-cover bg-[#152033]" /></div>
                        </div>
                        <p className="font-bold text-sm">accrualconsulting</p>
                    </div>
                    ...
                    <div className="p-3">
                        ...
                        <p className="text-sm"><span className="font-bold mr-1">accrualconsulting</span><span className="text-gray-700">{event.caption?.substring(0, 60)}...</span></p>
                    </div>
                </div>
            )}
            {event.platform === 'facebook' && (
                <div>
                    <div className="flex items-center gap-3 p-3">
                        <div className="w-10 h-10 rounded-full bg-[#152033] overflow-hidden relative"><img src="/logo192.png" alt="Accrual" className="w-full h-full object-cover absolute top-0 left-0" /></div>
                        <div><p className="font-bold text-[15px] leading-tight">Accrual</p><p className="text-xs text-gray-500">Publicado • Hace 2 min • 🌎</p></div>
                    </div>
            ...
            {event.platform === 'tiktok' && (
                <div className="relative bg-[#152033] text-white h-[350px] flex items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute right-2 bottom-12 flex flex-col items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-white border-2 border-white overflow-hidden shadow-lg"><img src="/logo192.png" className="w-full h-full object-cover bg-[#152033]" alt="logo" /></div>
                    ...
                    <div className="absolute bottom-4 left-4 right-16">
                        <p className="font-bold text-sm drop-shadow-md">@accrualconsulting</p>
    ...
    const renderSidebar = () => {
        if (canCreate) {
            return (
                <div className="w-[280px] border-l border-white/10 bg-[#152033]/40 backdrop-blur-2xl flex flex-col">
                    <div className="p-5 border-b border-white/10 bg-[#152033]/60">
                        ...
                        <div className="flex flex-col gap-2">
                            <select value={trendsNetwork} onChange={e => setTrendsNetwork(e.target.value)} className="w-full bg-[#152033] border border-white/20 text-white text-[10px] rounded p-1.5 focus:outline-none focus:border-[#0099CC]">
                                <option value="Todas">🌐 Todas las redes</option>
                                <option value="TikTok">⚫ TikTok</option>
                                <option value="Instagram">🟣 Instagram</option>
                                <option value="LinkedIn">🔵 LinkedIn</option>
                            </select>
                            <input value={trendsNiche} onChange={e => setTrendsNiche(e.target.value)} placeholder="Ej: SaaS, E-commerce, IA..." className="w-full bg-[#152033] border border-white/20 text-white text-[10px] rounded p-1.5 focus:outline-none focus:border-[#0099CC]" />
                            <button onClick={() => fetchTrends(trendsNetwork, trendsNiche)} disabled={loadingTrends} className="w-full bg-[#0099CC] text-white text-[10px] font-black uppercase py-1.5 rounded disabled:opacity-50 hover:bg-white hover:text-[#0099CC] transition-colors">
                    ...
                    <div className="flex-1 p-5 overflow-y-auto space-y-5">
                        {loadingTrends ? (
                            ...
                            <>
                                <div className="bg-[#152033]/50 border border-white/10 p-4 rounded-xl">
                                    <h4 className="text-[#0099CC] font-black text-[10px] uppercase mb-3">Trending {realTrends.niche} hoy:</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {(realTrends.hashtags || []).map(tag => (
                                            <span key={tag} onClick={() => setActiveHashtag(activeHashtag === tag ? null : tag)}
                                                className={`text-[10px] font-bold px-2 py-1 rounded cursor-pointer transition-colors ${activeHashtag === tag ? 'bg-gradient-to-r from-[#0099CC] to-blue-800 text-white' : 'text-neutral-300 bg-[#152033]/60 border border-white/10 hover:bg-white/20'}`}>
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                                    ...
                                    <ul className="text-[10px] text-neutral-300 space-y-3 font-bold leading-tight">
                                        {(realTrends.hooks || []).map((hk, i) => (
                                            <li key={i} className="flex gap-2">
                                                <span className="text-[#0099CC] shrink-0">👉</span><span>{hk}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
        ...
        return (
            <div className="w-[300px] border-l border-white/10 bg-[#152033]/40 backdrop-blur-2xl flex flex-col">
                <div className="p-5 border-b border-blue-900/50 bg-gradient-to-r from-[#0099CC]/10 to-transparent">
                    <h3 className="text-[#0099CC] font-black uppercase text-sm tracking-widest">🔔 Mis Tareas</h3>
                    <p className="text-xs text-blue-500/70 font-bold">Asignadas por CM / Dirección</p>
                </div>
                <div className="flex border-b border-white/10">
                    {[['pendientes', 'Por Realizar'], ['realizadas', 'Realizadas'], ['aprobadas', 'Listas']].map(([key, label]) => (
                        <button key={key} onClick={() => setTaskView(key)}
                            className={`flex-1 py-3 text-xs font-black uppercase tracking-widest transition-colors ${taskView === key ? 'text-[#0099CC] border-b-2 border-[#0099CC]' : 'text-neutral-600 hover:text-neutral-400'}`}>
                            {label}
                        </button>
                    ))}
                </div>
                    ...
                    {taskView === 'pendientes' && (
                        pendingTasks.length === 0
                            ...
                            : pendingTasks.map(task => (
                                <div key={task.id} className="bg-[#152033]/30 border border-blue-900/40 hover:border-blue-500/60 p-4 rounded-xl transition-colors">
                                    ...
                                    <div className="space-y-1 mt-2 border-t border-white/5 pt-2">
                                        ...
                                        <p className="text-[10px] text-neutral-500 font-bold">👤 Asignó: <span className="text-[#0099CC]">{task.asignadoPor}</span></p>
                                    </div>
                                    <button onClick={() => navigate('/studio')} className="mt-3 w-full text-[10px] bg-[#152033]/60 border border-neutral-700 text-[#0099CC] px-3 py-1.5 rounded-lg font-black hover:bg-[#0099CC] hover:text-white transition-all">Ir al Estudio ➔</button>
                                </div>
                            ))
                    )}
                    {taskView === 'realizadas' && (
                                ...
                                return (
                                    <div key={task.id} className="group bg-yellow-950/20 border border-yellow-500/30 hover:border-yellow-500/60 p-4 rounded-xl transition-colors cursor-pointer relative overflow-hidden" onClick={() => { setSelectedPublishTask(task); setShowPublishModal(true); setPublishNetwork('instagram'); }}>
                                        ...
                                        <div className="flex gap-3 mb-2">
                                            <div className="w-12 h-12 bg-[#152033] rounded shrink-0 overflow-hidden relative">
    ...
    return (
        <div
            className="h-full w-full bg-[#233657] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(0,153,204,0.1),rgba(255,255,255,0))] relative flex overflow-hidden text-white"
            onClick={(e) => {
        ...
        >
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                {/* ── HEADER ── */}
                <div className="px-8 py-4 bg-[#152033]/60 backdrop-blur-xl border-b border-white/[0.06] shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative" data-dropdown="notifications">
                                <button onClick={() => { setShowNotifications(!showNotifications); setNotifications(prev => prev.map(n => n.to?.toLowerCase() === currentUser.toLowerCase() ? { ...n, read: true } : n)); }}
                                    ...
                                    {unreadCount > 0 && (
                                        <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#0099CC] rounded-full text-[10px] font-black flex items-center justify-center text-white animate-pulse shadow-[0_0_8px_rgba(0,153,204,0.8)]">
                                            {unreadCount}
                                        </span>
                                    )}
                                </button>
                                                ...
                                                : notifications.filter(n => n.to?.toLowerCase() === currentUser.toLowerCase()).map(n => (
                                                    <div key={n.id} className={`p-4 border-b border-white/5 hover:bg-white/5 transition-colors ${!n.read ? 'border-l-2 border-l-[#00ff88]' : ''}`}>
                                                        <div className="flex justify-between items-start mb-1">
                                                            <span className="text-[#0099CC] font-black text-xs">{n.from}</span>
                            ...
                            {canCreate && calendarTab !== 'pendientes' && (
                                <button onClick={() => setShowNewAssignModal(true)}
                                    className="px-4 py-2 bg-white/[0.05] backdrop-blur-sm border border-white/10 hover:border-[#0099CC]/40 text-[#0099CC] rounded-xl font-black text-xs transition-all uppercase tracking-widest">
                                    📋 Asignar Tarea
                                </button>
                            )}
                            ...
                            {canCreate && calendarTab !== 'pendientes' && (
                                <button onClick={() => setShowNewCampaignModal(true)}
                                    className="px-4 py-2 bg-gradient-to-r from-[#0099CC] to-blue-800 hover:from-white hover:to-white hover:text-[#0099CC] text-white rounded-xl font-black text-xs transition-all shadow-[0_4px_15px_rgba(0,153,204,0.4)] uppercase tracking-widest flex items-center gap-1">
                                    ➕ Campaña
                                </button>
                            )}
                        </div>
                        ...
                        {[
                            ...
                            <button key={tab.id} onClick={() => setCalendarTab(tab.id)}
                                className={`px-4 py-2 rounded-xl font-black text-xs transition-all flex items-center gap-1.5 ${
                                    calendarTab === tab.id
                                        ? 'bg-[#0099CC] text-white shadow-[0_0_12px_rgba(0,153,204,0.4)]'
                                ...
                                <div className="flex bg-white/[0.04] border border-white/[0.06] rounded-xl overflow-hidden">
                                    {['month', 'week'].map(v => (
                                        <button key={v} onClick={() => setCalendarView(v)}
                                            className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-colors ${calendarView === v ? 'bg-[#0099CC] text-white' : 'text-neutral-500 hover:text-white'}`}>
                                            {v === 'month' ? '🗓 Mes' : '📅 Semana'}
                                        </button>
                                    ))}
                                </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                    ...
                    {canCreate && (calendarTab === 'contenido' || calendarTab === 'todos') && activePlatform !== 'ALL' && (
                        <div className="mx-6 mt-4 mb-2 bg-[#0a0a0a]/80 backdrop-blur-sm border border-[#0099CC]/30 rounded-xl p-4 shadow-[0_0_20px_rgba(0,153,204,0.1)] shrink-0">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button onClick={saveBotConfig} disabled={savingBot || !botConfig}
                                    className="bg-[#0099CC] hover:bg-white hover:text-[#0099CC] text-white text-[10px] font-black uppercase px-4 py-1.5 rounded transition-colors disabled:opacity-50">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                                    <div>
                                        ...
                                        <input type="text" value={botConfig.keywords}
                                            ...
                                            className="w-full bg-[#152033]/60 border border-white/10 rounded p-2 text-white font-bold focus:outline-none focus:border-[#0099CC] transition-colors"
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <textarea value={botConfig.dm_system_prompt || ''}
                                            ...
                                            className="w-full h-14 bg-[#152033]/60 border border-[#0099CC]/20 rounded p-2 text-yellow-300/80 font-mono text-[10px] focus:outline-none focus:border-[#0099CC] transition-colors resize-none"
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden">
                            ...
                            {calendarTab === 'pendientes' ? (
                                <div className="flex bg-[#0a0a0a] border border-neutral-800 rounded-2xl overflow-hidden flex-1 m-4 font-sans shadow-2xl">
                                    {/* Left Pane */}
                                    <div className="w-1/2 md:w-5/12 border-r border-neutral-800 flex flex-col bg-[#233657] text-white">
                                        <div className="flex items-center px-4 py-3 border-b border-neutral-800 shrink-0 bg-[#0a0a0a] relative" data-dropdown="templates">
                                            <button onClick={() => setShowTemplateDropdown(!showTemplateDropdown)} className="bg-[#0099CC] hover:bg-blue-800 text-white rounded-lg px-4 py-1.5 text-xs font-black uppercase tracking-widest flex items-center shadow-[0_0_15px_rgba(0,153,204,0.3)] transition-all relative z-10">
                                            ...
                                            {showTemplateDropdown && (
                                                <div className="absolute top-12 left-4 w-64 bg-[#111] border border-neutral-700 rounded-xl shadow-2xl z-50 overflow-hidden text-white font-sans">
                                                    <button onClick={() => { setShowTemplateDropdown(false); setNewTask({ que: '', para: '', referencias: '', deadline: '', audience: 'Marketing', priority: 'Medium', contentType: 'Backlog' }); setShowNewAssignModal(true); }}
                                                        ...
                                                        <span className="text-[#0099CC] text-sm">+</span> Blank task
                                                    </button>
                                                    ...
                                                    <div className="flex flex-col">
                                                        <button onClick={() => { setShowTemplateDropdown(false); setNewTask({ que: 'Rebrand Outreach Campaign', para: 'Alex', referencias: 'Objetivo: Renovación visual de pautas Q4.', deadline: new Date(Date.now() + 86400000 * 7).toISOString().split('T')[0], audience: 'Social Media', priority: 'High', contentType: 'Launch' }); setShowNewAssignModal(true); }}
                                                            className="w-full text-left px-4 py-2.5 text-[11px] font-bold hover:bg-[#0099CC]/10 hover:text-[#0099CC] transition-colors flex items-center gap-2 group text-neutral-300">
                                            ...
                                            <div className="flex flex-col mt-1">
                                                {tasks.length === 0 ? (
                                                    ...
                                                    <div key={task.id} onClick={() => setSelectedTaskBoard(task)}
                                                        className={`flex items-center border-b border-neutral-800/50 px-4 py-2 cursor-pointer max-h-12 transition-all ${selectedTaskBoard?.id === task.id ? 'bg-[#0099CC]/10 border-l-[3px] border-l-[#0099CC]' : 'hover:bg-white/5 border-l-[3px] border-l-transparent'}`}>
                                                        <div onClick={(e) => { e.stopPropagation(); setTasks(prev => prev.map(t => t.id === task.id ? { ...t, done: !t.done } : t)); }}
                                                            className={`flex-shrink-0 w-5 h-5 rounded-md border flex items-center justify-center mr-3 cursor-pointer transition-colors ${task.done ? 'bg-green-500/20 border-green-500 text-green-500' : 'bg-[#152033] border-neutral-600 hover:border-green-500 hover:text-green-500'}`}>
                                                        ...
                                                        <div className="hidden xl:flex items-center space-x-1.5 mr-3 shrink-0">
                                                            ...
                                                            <div className={`rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wider w-14 text-center border ${task.priority === 'High' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : task.priority === 'Low' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 'bg-orange-500/10 text-orange-400 border-orange-500/20'}`}>{task.priority || 'Medium'}</div>
                                                        </div>
                                            ...
                                            <div className="flex-1 flex flex-col overflow-hidden">
                                                ...
                                                <div className="flex-1 overflow-y-auto px-8 py-8">
                                                    <div className="flex items-center text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-6 bg-neutral-900/50 w-max px-3 py-1.5 rounded-lg border border-neutral-800">
                                                        <span className="mr-2 text-[#0099CC]">🔒</span>
                                                    ...
                                                    <div className="pt-2">
                                                        ...
                                                        <div className="flex space-x-4">
                                                            <div className="w-10 h-10 rounded-full bg-[#0099CC] flex items-center justify-center text-sm font-black text-white shrink-0 uppercase border-2 border-blue-900">{(currentUser || 'U')[0]}</div>
                                                            <div className="flex-1 border border-neutral-800 hover:border-neutral-600 rounded-xl bg-[#111] p-0 shadow-inner flex flex-col focus-within:border-[#0099CC]/50 transition-colors">
                                    ...
                                    {loadingEvents ? (
                                        ...
                                        events.length === 0 ? (
                                            <div className="flex flex-col items-center justify-center flex-1 opacity-40 gap-3 py-20">
                                                ...
                                                {canCreate && (
                                                    <button onClick={() => setShowNewCampaignModal(true)}
                                                        className="mt-2 px-6 py-2 bg-[#0099CC] text-white font-black text-xs rounded-xl uppercase tracking-widest hover:bg-white hover:text-[#0099CC] transition-all">
                                                        + Nueva Campaña
                                                    </button>
                                                )}
                                            </div>
            ...
            {selectedEvent && (
                <div className="absolute top-0 right-0 h-full w-[420px] bg-[#152033]/50 backdrop-blur-2xl border-l border-white/[0.06] shadow-[0_0_50px_rgba(0,0,0,0.9)] flex flex-col z-50">
                    <div className={`p-4 border-b flex justify-between items-center shrink-0 ${selectedEvent.status === 'urgent' ? 'bg-gradient-to-r from-[#0099CC] to-blue-800 border-blue-900' : 'bg-[#152033]/30 border-white/[0.06]'}`}>
                        <div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-5">
                        ...
                        <div>
                            ...
                            {canCreate
                                ? <textarea defaultValue={selectedEvent.caption} className="w-full bg-[#152033]/60 border border-white/10 p-3 rounded-xl text-sm text-white outline-none focus:border-yellow-500 transition-colors resize-none" rows="3" />
                                : <p className="bg-[#152033]/40 border border-white/5 p-3 rounded-xl text-sm text-neutral-300 font-bold">{selectedEvent.caption}</p>
                            }
                        </div>
                        ...
                        {canCreate && (
                            <div className="pt-3 border-t border-white/[0.06]">
                                ...
                                <div className="space-y-2 mb-3">
                                    <input type="text" placeholder="¿El qué? (Ej: Oscurecer imagen)" value={correctionForm.que} onChange={e => setCorrectionForm({...correctionForm, que: e.target.value})} className="w-full bg-[#152033]/30 border border-blue-900/50 p-2.5 text-white text-xs rounded-lg outline-none focus:border-[#0099CC]" />
                                    <input type="text" placeholder="¿El cuándo? (Ej: Urgente, Hoy 5PM)" value={correctionForm.cuando} onChange={e => setCorrectionForm({...correctionForm, cuando: e.target.value})} className="w-full bg-[#152033]/30 border border-blue-900/50 p-2.5 text-white text-xs rounded-lg outline-none focus:border-[#0099CC]" />
                                    <input type="text" placeholder="¿Para qué? (Ej: Post de mañana IG)" value={correctionForm.paraQue} onChange={e => setCorrectionForm({...correctionForm, paraQue: e.target.value})} className="w-full bg-[#152033]/30 border border-blue-900/50 p-2.5 text-white text-xs rounded-lg outline-none focus:border-[#0099CC]" />
                                    <input type="url" placeholder="Referencias (URLs, Drive, etc.)" value={correctionForm.referencias} onChange={e => setCorrectionForm({...correctionForm, referencias: e.target.value})} className="w-full bg-[#152033]/30 border border-blue-900/50 p-2.5 text-white text-xs rounded-lg outline-none focus:border-[#0099CC]" />
                                    <textarea placeholder="Comentarios adicionales / @Menciones..." value={correctionForm.comentarios} onChange={e => setCorrectionForm({...correctionForm, comentarios: e.target.value})} className="w-full bg-[#152033]/30 border border-blue-900/50 p-2.5 text-white text-xs rounded-lg resize-none outline-none focus:border-[#0099CC]" rows="2" />
                                </div>
                                <button onClick={() => {
                                ...
                                }} className="w-full bg-gradient-to-r from-[#0099CC] to-blue-800 text-white font-black py-2.5 rounded-xl text-xs uppercase transition-all hover:from-blue-700 shadow-md">
                                    Mandar a Corregir ➔
                                </button>
                            </div>
                        )}
                        ...
                        <div className="pt-3 border-t border-white/[0.06]">
                                ...
                                {(selectedEvent.comments || []).length === 0
                                    ...
                                    : (selectedEvent.comments || []).map(c => (
                                        <div key={c.id} className={`flex gap-2 ${c.author?.toLowerCase() === currentUser.toLowerCase() ? 'flex-row-reverse' : ''}`}>
                                            <div className="w-7 h-7 rounded-full bg-[#0099CC]/20 border border-[#0099CC]/40 flex items-center justify-center shrink-0 text-[10px] font-black text-[#0099CC]">{(c.author || '?')[0].toUpperCase()}</div>
                                            <div className={`max-w-[75%] ${c.author?.toLowerCase() === currentUser.toLowerCase() ? 'items-end' : 'items-start'} flex flex-col`}>
                                                <div className={`px-3 py-2 rounded-xl text-xs font-bold leading-snug ${c.author?.toLowerCase() === currentUser.toLowerCase() ? 'bg-[#0099CC]/20 border border-[#0099CC]/30 text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-neutral-300 rounded-tl-none'}`}>
                            ...
                            <div className="relative">
                                {mentionQuery !== null && (
                                    <div className="absolute bottom-full mb-1 left-0 bg-[#1a1a1a] border border-[#0099CC]/30 rounded-xl overflow-hidden shadow-xl z-10 min-w-[160px]">
                                        {TEAM.filter(u => u.toLowerCase().startsWith(mentionQuery.toLowerCase())).map(u => (
                                            <button key={u} onClick={() => insertMention(u)} className="w-full text-left px-4 py-2 text-sm font-black text-white hover:bg-[#0099CC]/20 transition-colors flex items-center gap-2">
                                                <span className="w-6 h-6 rounded-full bg-[#0099CC]/20 border border-[#0099CC]/40 flex items-center justify-center text-[10px] text-[#0099CC]">{u[0]}</span>
                                                @{u}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="flex gap-2">
                                    <input ref={commentInputRef} value={commentText} onChange={handleCommentChange}
                                        ...
                                        className="flex-1 bg-[#152033]/50 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#0099CC] transition-colors placeholder:text-neutral-700" />
                                    <button onClick={submitComment} className="bg-[#0099CC] hover:bg-blue-700 text-white font-black px-4 rounded-xl text-xs transition-colors shrink-0">↑</button>
                                </div>
                            </div>
                        </div>
            ...
            {showNewAssignModal && canCreate && (
                <div className="absolute inset-0 z-[100] flex items-center justify-center p-4 bg-[#152033]/80 backdrop-blur-sm">
                    <div className="bg-[#0d0d0d] border border-white/[0.06] p-8 rounded-2xl w-full max-w-lg shadow-[0_0_50px_rgba(0,153,204,0.15)] relative">
                        ...
                        <h3 className="text-xl font-black text-white tracking-widest uppercase mb-6 flex items-center gap-2"><span className="text-[#0099CC]">📋</span> Asignar Tarea</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-black text-neutral-500 uppercase mb-2">¿Qué se debe hacer? *</label>
                                <textarea value={newTask.que} onChange={e => setNewTask({ ...newTask, que: e.target.value })} placeholder="Ej: Hacer imagen menos oscura..." rows="3" className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#0099CC] transition-colors resize-none" />
                            </div>
                            <div>
                                <label className="block text-xs font-black text-neutral-500 uppercase mb-2">¿Para quién? *</label>
                                <select value={newTask.para} onChange={e => setNewTask({ ...newTask, para: e.target.value })} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#0099CC] transition-colors">
                            ...
                            <div>
                                ...
                                <div className="space-y-3 p-4 border border-white/5 bg-white/[0.02] rounded-xl">
                                    <input type="text" value={newTask.referencias} onChange={e => setNewTask({ ...newTask, referencias: e.target.value })} placeholder="Link, brief, notas o URL..." className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#0099CC] transition-colors" />
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <select value={newTask.audience} onChange={e => setNewTask({ ...newTask, audience: e.target.value })} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-3 py-2.5 text-xs font-bold text-emerald-400 focus:outline-none transition-colors">
                                ...
                                <div>
                                    ...
                                    <select value={newTask.priority} onChange={e => setNewTask({ ...newTask, priority: e.target.value })} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-3 py-2.5 text-xs font-bold text-orange-400 focus:outline-none transition-colors">
                                        <option value="High">High</option><option value="Medium">Medium</option><option value="Low">Low</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-[10px] font-black text-neutral-500 uppercase mb-2">Content Type</label>
                                    <select value={newTask.contentType} onChange={e => setNewTask({ ...newTask, contentType: e.target.value })} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-3 py-2.5 text-xs font-bold text-fuchsia-400 focus:outline-none transition-colors">
                            ...
                            <div>
                                ...
                                <input type="date" value={newTask.deadline} onChange={e => setNewTask({ ...newTask, deadline: e.target.value })} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#0099CC] transition-colors [color-scheme:dark]" />
                            </div>
                            ...
                            }} className="w-full bg-[#0099CC] hover:bg-white text-white hover:text-[#0099CC] py-4 rounded-xl font-black uppercase tracking-widest transition-all">
                                Asignar Tarea ✔️
                            </button>
                        </div>
                    </div>
            ...
            {showNewCampaignModal && canCreate && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#152033]/80 backdrop-blur-sm">
                    <div className="bg-[#0d0d0d] border border-white/[0.06] p-6 md:p-8 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-[0_0_50px_rgba(0,153,204,0.15)] relative">
                        ...
                        <h3 className="text-xl font-black text-white tracking-widest uppercase mb-6 flex items-center gap-2">
                            <span className="text-[#0099CC]">🎯</span> Programar Campaña
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select value={newCampaign.asignado} onChange={e => setNewCampaign({...newCampaign, asignado: e.target.value})} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#0099CC] transition-colors">
                                        <option value="">— Selecciona —</option>
                                        {TEAM.map(u => <option key={u} value={u}>@{u}</option>)}
                                    </select>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" value={newCampaign.titulo} onChange={e => setNewCampaign({...newCampaign, titulo: e.target.value})} placeholder="Ej: Video explicativo Accrual..." className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#0099CC] transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-xs font-black text-neutral-500 uppercase mb-2">Fecha *</label>
                                    <input type="date" value={newCampaign.fecha} onChange={e => setNewCampaign({...newCampaign, fecha: e.target.value})} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#0099CC] transition-colors [color-scheme:dark]" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-black text-neutral-500 uppercase mb-2">Plataforma *</label>
                                <select value={newCampaign.plataforma} onChange={e => setNewCampaign({...newCampaign, plataforma: e.target.value})} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#0099CC] transition-colors">
                                ...
                                <div className="space-y-2 border border-white/5 p-3 rounded-xl bg-white/[0.02]">
                                    {[['🖼️', 'urlFoto', 'URL de Foto/Drive...'], ['🎬', 'urlVideo', 'URL de TikTok/Reel...'], ['🔗', 'urlReferencia', 'URL al documento...']].map(([icon, key, ph]) => (
                                        <div key={key} className="flex items-center gap-3">
                                            ...
                                            <input type="url" value={newCampaign[key]} onChange={e => setNewCampaign({...newCampaign, [key]: e.target.value})} placeholder={ph} className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-[#0099CC] transition-colors" />
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <div>
                                ...
                                <textarea value={newCampaign.briefing} onChange={e => setNewCampaign({...newCampaign, briefing: e.target.value})} placeholder="Explica la visión, tono y activos necesarios..." rows="3" className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#0099CC] transition-colors resize-none" />
                            </div>
                            <button
                                onClick={handleCreateCampaign}
                                disabled={savingCampaign}
                                className="w-full bg-[#0099CC] hover:bg-white text-white hover:text-[#0099CC] py-4 rounded-xl font-black uppercase tracking-widest transition-all mt-4 border border-blue-900/50 shadow-[0_4px_15px_rgba(0,153,204,0.5)] disabled:opacity-50 flex items-center justify-center gap-2"
                            >
                                {savingCampaign ? (
            ...
            {showPublishModal && selectedPublishTask && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#152033]/90 backdrop-blur-md">
                    <div className="bg-[#0a0a0a] border border-[#d4af37]/30 p-0 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-[0_0_50px_rgba(212,175,55,0.15)] relative overflow-hidden">
                        ...
                        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
                            ...
                            <div className="w-full md:w-1/3 border-r border-neutral-800 bg-[#233657] p-5 flex flex-col overflow-y-auto">
                            ...
                            <div className="w-full md:w-2/3 bg-neutral-950 flex items-center justify-center p-6 md:p-10 relative">
                                ...
                                <div className="w-full max-w-[320px] shadow-2xl relative z-10 transition-all duration-300 transform rounded-[2rem] border-[6px] border-black bg-[#152033]">
                                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-6 bg-[#152033] rounded-b-xl z-50"></div>
            ...
            {showSheetsModal && (
                <div className="fixed inset-0 bg-[#152033]/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-[0_30px_80px_rgba(0,0,0,0.9)]">
                        ...
                        <div className="p-6 flex flex-col gap-4 flex-1 overflow-y-auto">
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <input type="text" value={sheetsUrl} onChange={e => setSheetsUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchSheetsPreview()} placeholder="https://docs.google.com/spreadsheets/d/..." className="flex-1 bg-[#152033] border border-white/20 text-white text-xs rounded-xl px-4 py-3 focus:outline-none focus:border-green-500/60 placeholder-neutral-700 font-mono" />
                                ...
                                <p className="text-[10px] text-neutral-600 mt-2 font-bold">
                                    ⚠️ Comparte el Sheet con: <span className="text-green-400 font-mono text-[9px]">zilla-calendar@bot-accrual.iam.gserviceaccount.com</span>
                                </p>
                            </div>

                            {sheetsError && <div className="bg-blue-950/50 border border-blue-800 rounded-xl p-4 text-blue-400 text-xs font-bold">❌ {sheetsError}</div>}
                            {sheetsPreview && (
