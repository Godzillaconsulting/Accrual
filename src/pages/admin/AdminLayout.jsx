...
import { Home, Users, Briefcase, FileText, Settings, LogOut, Menu, X, LayoutDashboard, MonitorPlay } from 'lucide-react';
...
const AdminLayout = () => {
    ...
    // Determinar la URL del preview basado en la ruta actual
    let previewUrl = null;
    if (location.pathname.includes('/admin/homepage')) previewUrl = '/';
    else if (location.pathname.includes('/admin/quienes-somos')) previewUrl = '/quienes-somos';
    else if (location.pathname.includes('/admin/servicios')) previewUrl = '/servicios';
    else if (location.pathname.includes('/admin/articulos')) previewUrl = '/articulos';
        ...
        <div className="flex bg-[#F3F4F6] min-h-screen text-gray-800 font-sans">
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden pt-16 md:pt-0 relative">
                ...
                <div className="flex-1 flex overflow-hidden z-10">
                    <div className={`flex-1 overflow-y-auto p-4 md:p-8 ${previewUrl ? 'pr-4 lg:pr-8 border-r border-gray-200' : ''}`}>
                    ...
                    {previewUrl && (
                        <div className="hidden lg:flex flex-col w-[450px] xl:w-[600px] bg-[#f8f9fa]">
                            <div className="h-10 bg-[#e2e8f0] border-b border-gray-300 flex items-center px-4 justify-between shadow-sm z-10">
                                <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full bg-red-400"></div>
                                    <div className="w-3 h-3 rounded-full bg-yellow-400"></div>
                                    <div className="w-3 h-3 rounded-full bg-green-400"></div>
                                </div>
                                <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider bg-white px-3 py-1 rounded-full shadow-sm">
                                    <MonitorPlay className="w-3 h-3 text-[#00D0B0]" />
                                    Live Preview
                                </div>
                                <div className="w-10"></div> {/* Spacer for centering */}
                            </div>
                            <div className="flex-1 p-4 bg-[#f1f5f9] overflow-hidden">
                                <div className="w-full h-full bg-white rounded-lg shadow-xl overflow-hidden border border-gray-200 relative group">
                                    <div className="absolute inset-0 bg-black/5 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
                                        <span className="bg-[#233657] text-white px-4 py-2 rounded-full text-xs font-medium shadow-lg backdrop-blur-md">
                                            Vista previa en vivo interactiva
                                        </span>
                                    </div>
                                    {/* Usamos el origin actual o ruta relativa para que funcione tanto en local como Vercel */}
                                    <iframe 
                                        src={previewUrl} 
