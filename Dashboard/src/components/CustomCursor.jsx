...
export default function CustomCursor() {
  ...
  const hiddenRoutes = ['/admin', '/cm', '/studio', '/login', '/dashboard', '/accrual-sora'];
    ...
    <div className={`fixed inset-0 pointer-events-none z-[99999] transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      {boxes.map((_, i) => (
        <div
          ...
          className="absolute top-0 left-0 w-[25px] h-[25px] rounded-full mix-blend-screen bg-[#0099CC]/20 pointer-events-none -translate-x-1/2 -translate-y-1/2"
      ...
      <div
        ...
        className="absolute top-0 left-0 w-3 h-3 bg-[#0099CC] border border-white/60 shadow-[0_0_8px_rgba(255,255,255,0.4)] rounded-full pointer-events-none -translate-x-1/2 -translate-y-1/2"
