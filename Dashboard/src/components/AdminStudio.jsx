...
function EditorField({ fieldKey, onHover, children }) {
 ...
 >
 <div className="absolute -left-2 top-0 bottom-0 w-0.5 bg-[#0099CC]/0 group-hover/field:bg-[#0099CC]/70 rounded-full transition-all duration-150" />
 {children}
 </div>
 );
}
...
function ColorField({ label, fieldKey, draftData, onChange }) {
 const val = draftData[fieldKey] ||'#0099CC';
 return (
 ...
 className="flex-1 p-2 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-lg text-white text-xs font-mono focus:border-[#0099CC] outline-none" />
...
export default function AdminStudio() {
 ...
 const [adminProfile, setAdminProfile] = useState(() => {
        try { return JSON.parse(localStorage.getItem('accrual_cached_profile')) || null; } catch { return null; }
 });
     ...
     if (token) {
         ...
         .then(data => { 
             if(data.success) {
                 ...
                 localStorage.setItem('accrual_cached_profile', JSON.stringify(data.profile));
             }
         })
 ...
 return (
 <div 
   className="fixed inset-0 z-50 flex bg-[#233657] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(0,153,204,0.15),rgba(255,255,255,0))] text-white font-sans overflow-hidden relative"
   onContextMenu={(e) => {
 ...
 {/* ── MODAL PUBLICAR ── */}
 {showPublishModal && (
 <div className="fixed inset-0 z-[100] bg-[#152033]/80 backdrop-blur-sm flex items-center justify-center p-4 md:p-6">
 ...
 <iframe src="https://accrual.ai" title="Preview" className="w-full h-[52vh] rounded-xl border border-neutral-700" />
 </div>
 ...
 <button onClick={handlePublish} disabled={isPublishing} className="px-7 py-2.5 bg-[#0099CC] hover:bg-white text-white hover:text-[#0099CC] disabled:opacity-50 rounded-full font-black text-sm transition-colors duration-300 shadow-[0_4px_20px_rgba(0,153,204,0.5)]">{isPublishing ? 'Publicando...' : 'Confirmar y Publicar'}</button>
 </div>
 </div>
 </div>
 )}
 ...
 <div className={`relative z-10 transition-all duration-300 flex flex-col border-r border-blue-900/30 bg-[#0099CC]/5 backdrop-blur-xl shadow-[4px_0_24px_rgba(0,0,0,0.05)] ${isSidebarOpen ? 'w-[200px] min-w-[200px]' : 'w-0 min-w-0 overflow-hidden opacity-0 pointer-events-none'}`}>
  ...
  <a href="/" target="_blank" rel="noopener noreferrer" className="flex items-center cursor-pointer hover:scale-110 transition-transform">
    <img src="/favicon.png" alt="Accrual Logo" className="w-8 h-8 drop-shadow-[0_2px_8px_rgba(0,153,204,0.6)]" />
  </a>
  </div>
 ...
 <>
 {/* Tabs */}
 <div className="flex gap-2 px-6 py-3 border-b border-[#0099CC]/40 bg-[#0099CC]/5 shrink-0 overflow-x-auto [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#0099CC]/50 hover:[&::-webkit-scrollbar-thumb]:bg-[#0099CC]/80 [&::-webkit-scrollbar-track]:bg-transparent">
 {tabs.map(tab => (
 <button key={tab.id}
 ...
 className="w-full p-3 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-xl text-white font-bold text-sm focus:bg-[#0099CC]/10 focus:border-[#0099CC]/50 outline-none resize-none transition-colors placeholder:text-neutral-600" />
 </div>
 </EditorField>
 );
 }
 ...
 className="w-full p-2 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-lg text-white text-xs focus:border-[#0099CC] outline-none resize-none" />
 </div>
 </EditorField>
 );
 })}
      ...
      <div className="space-y-4 mt-6 pt-4 border-t border-neutral-800">
          ...
          {(draftData.mediaGallery || []).map((mediaItem, idx) => (
              <div key={idx} className="bg-neutral-900 rounded-xl p-3 space-y-3 border border-neutral-800 relative">
                  ...
                  }} className="absolute top-3 right-3 text-xs text-blue-500 font-bold hover:text-blue-400">✕ Eliminar</button>
                  ...
                  <div className="space-y-1 mt-2">
                        ...
                        <select 
                            ...
                            className="w-full p-2 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-lg text-white text-xs focus:border-[#0099CC] outline-none"
                        >
 ...
 className="w-full p-2.5 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-lg text-white font-bold text-sm focus:bg-[#0099CC]/10 focus:border-[#0099CC]/50 outline-none">
 ...
 selectedElementIndex === idx ?'bg-neutral-700 border-[#0099CC] text-white' :'bg-neutral-800 border-transparent text-neutral-400 hover:text-white'
 }`}>
 ...
 className={`w-full text-left px-2.5 py-2 rounded-lg text-xs transition border ${
 selectedFeatureIndex === idx ?'bg-neutral-700 border-[#0099CC] text-white' :'bg-neutral-800 border-transparent text-neutral-400 hover:text-white'
 }`}>
 ...
 className="mt-2 w-full text-center px-2 py-2 border border-dashed border-neutral-700 text-neutral-500 rounded-lg text-xs font-semibold hover:text-[#0099CC] hover:border-[#0099CC] transition transition-all duration-300">
 ...
 {activeElement && (
 <>
 <p className="text-xs font-bold text-[#0099CC]">Editando elemento</p>
 ...
 className="w-full p-2.5 bg-[#152033]/40 backdrop-blur-md border border-[#0099CC]/20 shadow-inner rounded-xl text-white font-bold text-sm focus:bg-[#0099CC]/10 focus:border-[#0099CC]/50 outline-none resize-none" />
 </div>
 </EditorField>
 );
 })}
 ...
 {activeFeature && (
 <>
 <p className="text-xs font-bold text-[#0099CC]">Editando característica</p>
 ...
 }} className="w-full mt-4 px-4 py-3 bg-blue-900/10 text-blue-500 text-sm font-bold rounded-xl border border-blue-900/30 hover:bg-blue-900/30 hover:border-blue-900/50 transition">
 ...
 {showFeedbackModal && (
 <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-[#152033]/80 backdrop-blur-sm">
   <div className="bg-[#111111] border border-white/10 p-8 rounded-2xl w-full max-w-md shadow-[0_0_50px_rgba(0,153,204,0.2)] relative">
     ...
     <p className="text-xs text-neutral-400 font-bold mb-4">Envía tus sugerencias, pide funciones o reporta bugs para <span className="text-[#0099CC]">Dani</span> y <span className="text-[#0099CC]">JareG</span>.</p>
     <textarea value={feedbackText} onChange={e => setFeedbackText(e.target.value)} placeholder="Ej: Un botón para descargar imágenes está fallando..." rows="4" className="w-full bg-[#152033]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors resize-none mb-3" />
