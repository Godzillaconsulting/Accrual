...
const bgVideoServicios = 'https://bot.accrual.ai/api/media/assets/Particulas Rojas.mp4';
const bgVideoCultura = 'https://bot.accrual.ai/api/media/assets/Particulas Rojas.mp4';
const gifBot = 'https://bot.accrual.ai/api/media/assets/Bot.gif';
const gifVideo = 'https://bot.accrual.ai/api/media/assets/Video.gif';
const gifEmbudo = 'https://bot.accrual.ai/api/media/assets/Embudo.gif';
const gifRedes = 'https://bot.accrual.ai/api/media/assets/Redes Sociales.gif';
const gifSeo = 'https://bot.accrual.ai/api/media/assets/Red Social Optimizar.gif';
const gifCrm = 'https://bot.accrual.ai/api/media/assets/Estadistica.gif';
...
export function injectSectionDefaults(nodeId, draftSource) {
  ...
  if (nodeId === 'recursos') {
      ...
      if (combinedData.recurso2EmailSubject === undefined) {
          ...
          combinedData.recurso2FileUrl = "https://accrual.ai/lazaro.pdf";
      }
      if (combinedData.recurso3EmailSubject === undefined) {
          ...
          combinedData.recurso3EmailBody = "Hola,\n\nGracias por solicitar el Tablero de Control de Ventas. Accede inmediatamente al recurso en el siguiente enlace:\n\nEste documento ha sido estructurado para ayudarte a detectar fugas operativas en tu embudo y recuperar hasta el 30% de tus ventas perdidas. Sugerimos prestar especial atención a la pestaña *Semáforo de Leads* para gestionar contactos de manera oportuna.\n\nTen en cuenta que este tablero te proporciona el diagnóstico y el mapa. Si requieres infraestructura tecnológica para automatizar el seguimiento y el cierre, en Accrual nos especializamos en instalar motores de IA para acelerar tus procesos.\n\nQuedamos a tu disposición para cualquier consulta.\n\nAtentamente,\nEl equipo de Accrual";
          combinedData.recurso3FileUrl = "https://accrual.ai/tablero.pdf";
      }
  }

  // Inject defaults for Socio Accrual Landing
  if (nodeId === 'socio-accrual') {
      if (combinedData.heroText === undefined) combinedData.heroText = 'Accrual Business Insights';
  ...
  if (nodeId === 'paquete-expansion') {
      if (combinedData.heroTitle === undefined) {
          ...
          combinedData.heroDisclaimer = 'Si en <span className="font-bold text-[#0099CC] not-italic">30 días hábiles</span>, no generamos ningún lead, te devolvemos el <span className="font-bold text-[#0099CC] not-italic">100%</span> de tu dinero.';
  ...
  if (nodeId === 'paquete-posicionamiento-social') {
      if (combinedData.heroTitle === undefined) {
          ...
          combinedData.heroDisclaimer = 'Si en <span class="font-bold text-white not-italic">14 días</span> no ves un incremento real en el engagement y la calidad de tu marca, el siguiente mes es <span class="font-bold text-[#0099CC] not-italic">GRATIS</span>.';
  ...
  if (nodeId === 'footer') {
      ...
      if (combinedData.contactEmail === undefined) combinedData.contactEmail = 'info@accrual.ai';
      ...
      if (combinedData.copyrightText === undefined) combinedData.copyrightText = 'Accrual Co. Todos los derechos reservados.';
  }
