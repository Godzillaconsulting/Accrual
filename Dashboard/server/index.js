...
const allowedOrigins = [
    ...
    'https://accrualconsulting.ai',
    'https://www.accrualconsulting.ai',
    'https://bot.accrualconsulting.ai',
];
...
app.use(cors({
    origin: function (origin, callback) {
        ...
        if (allowedOrigins.indexOf(origin) !== -1 || origin.includes('localhost') || origin.includes('vercel.app')) {
            ...
            callback(new Error('Bloqueado por CORS de Accrual: Origen no autorizado.'));
        }
    },
...
app.use('/api/sora/media', express.static('E:/AccrualSora_Outputs', { maxAge: '1y', immutable: true }));
...
// Cross-Origin-Resource-Policy: cross-origin → permite carga de imágenes/GIFs cross-origin (accrualconsulting.ai → bot.accrualconsulting.ai)
...
// Esto independiza totalmente a Accrual de Vercel (Host Autónomo)
const distPath = path.join(__dirname, '..', 'dist');
app.use(express.static(distPath));

// Endpoint de prueba para estado backend
app.get('/api', (req, res) => res.send('Accrual API Activa 🦖'));
