...
router.get('/download/:id', async (req, res) => {
    try {
        ...
        if (req.headers.host && req.headers.host.includes('accrualconsulting.ai') && !req.headers.host.includes('bot.')) {
            // Si el request vino por Vercel, redirigir al Túnel Directo
            return res.redirect(`https://bot.accrualconsulting.ai/api/premium/download/${newsletterId}?lang=${reqLang || ''}`);
        }

        // PAYWALL INTERCEPTOR (EN VIVO) 🔒
        // [DESACTIVADO TEMPORALMENTE A PETICIÓN]
        /* if (!isPreview) {
            const frontendUrl = process.env.FRONTEND_URL || 'https://accrualconsulting.ai';
        ...
        res.setHeader('Content-Disposition', `inline; filename="Accrual-Premium-Report-${reqLang}.pdf"`);
        res.send(pdfBuffer);
