...
router.post('/', requireAdmin, requireSuperAdmin, async (req, res) => {
    ...
    try {
        const salt = await bcrypt.genSalt(10);
        const hash = await bcrypt.hash(newPassword || 'Accrual2026', salt);
...
router.put('/:id', requireAdmin, requireSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        // Evitar degradar al fundador JareG, excepto para el dueño supremo (ID 2 - accrual_admin)
        if (cur.username === 'JareG' && isSuperadmin === false && parseInt(req.admin.id) !== 2) {
            return res.status(403).json({ success: false, message: 'El perfil de Fundador (JareG) solo puede ser degradado por el CEO (accrual_admin).' });
        }
...
router.delete('/:id', requireAdmin, requireSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        if (curRes.rows[0].username === 'JareG' && parseInt(req.admin.id) !== 2) {
            return res.status(403).json({ success: false, message: 'Infranqueable: Solo el CEO (accrual_admin) puede eliminar al perfil Fundador (JareG).' });
        }
