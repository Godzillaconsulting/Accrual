...
const TEMP_DIR = process.env.VERCEL ? '/tmp/uploads' : path.join(os.tmpdir(), 'accrual-uploads');
...
try {
    ...
    if (e.code === 'EROFS' || e.code === 'EACCES' || e.code === 'ENOENT') {
        ...
        ARCHIVOS_PESADOS_DIR = path.join(os.tmpdir(), 'accrual-assets');
...
const getPublicUrl = (req, relativePath) => {
    ...
    // Así la landing lo carga transparente en cualquier dominio (localhost o accrualconsulting.ai).
