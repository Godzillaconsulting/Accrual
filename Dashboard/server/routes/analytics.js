...
router.get('/dashboard', requireAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            const targetBots = ['accrual-bot-ig', 'tiktok-bot', 'whatsapp-bot', 'accrual-sora-engine'];
            botHealth = pm2Data
    ...
    try {
        if (network === 'fb' || network === 'messenger') {
            ...
            const cleanPosts = (data.data || []).map(p => ({
                ...
                url: p.permalink_url || 'https://facebook.com/accrual.consulting'
            }));
        ...
        if (network === 'ig' || network === 'ig_reels') {
            ...
            const cleanPosts = igMedia.map(p => ({
                ...
                url: p.permalink || 'https://instagram.com/accrual.consulting'
            }));
            return res.json({ success: true, posts: cleanPosts });
        } 
        ...
        if (network === 'tiktok') {
                ...
                try {
                    ...
                    if (!data.error && data.data && data.data.videos) {
                        const cleanPosts = (data.data.videos || []).map(v => ({
                            ...
                            url: v.share_url || 'https://tiktok.com/@accrualconsulting'
                        }));
                        return res.json({ success: true, posts: cleanPosts });
                    }
            ...
            const mockTikToks = [
                { id: "tk1", title: "El secreto para vender más 📈🔥", views: "15K", likes: 1240, comments: 89, url: "https://tiktok.com/@accrualconsulting" },
                { id: "tk2", title: "Por qué tu agencia no escala 🚫", views: "8.2K", likes: 640, comments: 42, url: "https://tiktok.com/@accrualconsulting" },
                { id: "tk3", title: "Neuromarketing 101 para Landing Pages 🧠", views: "22K", likes: 3100, comments: 210, url: "https://tiktok.com/@accrualconsulting" },
                { id: "tk4", title: "Cómo crear ofertas irresistibles 💸", views: "5.1K", likes: 380, comments: 15, url: "https://tiktok.com/@accrualconsulting" },
                { id: "tk5", title: "Automatiza tu WhatsApp en 3 pasos 🤖", views: "42K", likes: 5600, comments: 450, url: "https://tiktok.com/@accrualconsulting" }
            ];
