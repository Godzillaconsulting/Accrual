...
// La llave pública está en Cloudflare DNS: accrual._domainkey.accrualconsulting.ai
const getDkimConfig = () => {
    const privateKey = process.env.DKIM_PRIVATE_KEY;
    if (!privateKey) return undefined;

    return {
        domainName:  process.env.DKIM_DOMAIN    || 'accrualconsulting.ai',
        keySelector: process.env.DKIM_SELECTOR  || 'accrual',
...
const createTransporter = () => {
    ...
    if (process.env.EMAIL_DIRECT === 'true') {
        ...
        config = {
            ...
            port:   25,
            name:   process.env.DKIM_DOMAIN || 'accrualconsulting.ai', // HELO hostname
...
const bulkHeaders = (unsubUrl) => ({
    ...
    'Precedence':            'bulk',
    'X-Mailer':              'AccrualConsulting-Mailer/1.0',
});
    ...
    while (retries >= 0) {
        try {
            ...
            const result = await transporter.sendMail({
                from: `"${process.env.EMAIL_FROM_NAME || 'Accrual Consulting'}" <${process.env.EMAIL_FROM_ADDRESS || process.env.EMAIL_USER}>`,
                to,
...
export const sendNewsletterEmail = async ({ to, subject, bodyHtml, attachmentUrl }) => {
    const unsubUrl = `https://accrualconsulting.ai/api/newsletter/unsubscribe?email=${encodeURIComponent(to)}`;
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background-color:#111111;border:1px solid #222222;border-radius:12px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.8);">
        ...
        <tr>
          <td style="padding:30px 0;text-align:center;background:#09090b;border-bottom:3px solid #CC0000;">
             <img src="https://accrualconsulting.ai/favicon.png" alt="Accrual Consulting" width="55" height="55" style="display:block;margin:0 auto;" />
             <h1 style="margin:15px 0 0 0;font-size:18px;color:#fff;letter-spacing:2px;text-transform:uppercase;">Accrual Consulting</h1>
        ...
        <tr>
          <td style="padding:40px 30px;background-color:#09090b;border-top:1px solid #222222;text-align:center;">
             <div style="margin-bottom:20px;">
                <a href="https://www.instagram.com/accrual_consulting/" style="margin:0 10px;color:#888;text-decoration:none;font-size:14px;font-weight:bold;">Instagram</a>
                <span style="color:#444;">|</span>
                <a href="https://www.facebook.com/accrualjuarez" style="margin:0 10px;color:#888;text-decoration:none;font-size:14px;font-weight:bold;">Facebook</a>
                <span style="color:#444;">|</span>
                ...
                <span style="color:#444;">|</span>
                <a href="https://www.tiktok.com/@accrual_co_jrz" style="margin:0 10px;color:#888;text-decoration:none;font-size:14px;font-weight:bold;">TikTok</a>
             </div>
            <p style="font-size:11px;color:#666666;margin:0;line-height:1.6;">
              <strong>© ${new Date().getFullYear()} Accrual Consulting LLC.</strong> Todos los derechos reservados.<br/>
...
export const sendCitaConfirmationEmail = async ({ nombre, email, fecha, hora, tipoSesion, personalCalendarLink }) => {
            ...
            <tr>
              <td style="background:#f9f9f9;padding:20px 40px;border-top:1px solid #eee;text-align:center;">
                <p style="font-size:12px;color:#888;margin:0;">
                  © ${new Date().getFullYear()} Accrual Consulting — Ciudad Juárez, Chih.<br/>
                  <a href="https://accrualconsulting.ai" style="color:#CC0000;text-decoration:none;">accrualconsulting.ai</a>
                </p>
              </td>
            </tr>
          </table>
    ...
    const textPlain = `¡Cita confirmada, ${nombre}!\n\nTipo: ${tipoSesion}\nFecha: ${fechaLabel}\nHora: ${hora}\n\n${personalCalendarLink ? 'Agrega la cita a tu calendario: ' + personalCalendarLink : ''}\n\nAccrual Consulting — accrualconsulting.ai`;

    try {
        const transporter = createTransporter();
        const result = await transporter.sendMail({
            from:    `"Accrual Consulting 🦖" <${process.env.EMAIL_FROM_ADDRESS || process.env.EMAIL_USER}>`,
            to:      email,
            subject: `✅ Cita confirmada — ${tipoSesion} | Accrual Consulting`,
            text:    textPlain,
            html,
        });
