...
export async function buildPremiumPDF(data, lang = 'es') {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fillColor('#FFFFFF').fontSize(26).font('Helvetica-Bold').text('ACCRUAL', 105, 45, { letterSpacing: 1 });
            ...
            for (let i = 0; i < pages.count; i++) {
                ...
                doc.fillColor('#888888').fontSize(8).font('Helvetica').text(
                    `Propiedad Privada Integral de Accrual Consulting AI • ${ftrPag} ${i + 1} / ${pages.count} • ${ftrProd} ${new Date().toLocaleDateString()}`,
