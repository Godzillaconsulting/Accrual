...
export const wafMiddleware = (req, res, next) => {
    ...
    if (blockedIPs.has(ip)) {
        ...
        if (now < banData.expiration) {
            return res.status(403).json({ error: '🚨 IP Bloqueada temporalmente por Accrual WAF.' });
        } else {
            blockedIPs.delete(ip);
        }
    }
    ...
    if (riskLevel) {
        ...
        return res.status(403).json({ error: `🚨 Acceso Denegado por Accrual WAF. Detección: ${categoryName}` });
    }
