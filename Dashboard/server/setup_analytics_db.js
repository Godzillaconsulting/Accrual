...
async function setupAnalyticsDB() {
    ...
    try {
        ...
        // 4. GRANT PERMISSIONS (Assuming 'accrual_backend' role exists, like in create_leads_table)
        try {
            await pool.query('GRANT SELECT, INSERT, UPDATE ON page_views TO accrual_backend;');
            await pool.query('GRANT SELECT, INSERT, UPDATE ON video_retention TO accrual_backend;');
            console.log('✅ Permisos otorgados al rol backend.');
        } catch (e) {
            console.warn('⚠️ No se pudieron otorgar permisos al rol accrual_backend. Ignorar si no usas roles separados.', e.message);
        }
