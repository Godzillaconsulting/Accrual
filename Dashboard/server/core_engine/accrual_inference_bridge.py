...
OUTPUTS_DIR = os.environ.get("OUTPUTS_DIR_FALLBACK", r"E:\AccrualSora_Outputs")
...
lib_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f'accrual_tensor_manager{lib_ext}'))
...
app = FastAPI(title="Accrual AI Local GPU Node")
...
def load_ai_model():
    ...
    if ai_pipeline is None and AI_ENABLED:
        try:
            ...
            OFFLOAD_DIR = os.environ.get("OFFLOAD_DIR_FALLBACK", r"E:\AccrualSora_Offload")
            ...
            print("[SYSTEM] ACCRUAL SORA (PyTorch Engine) ONLINE. Listo para Producción en Cascada Limitada.")
        except Exception as e:
...
if __name__ == "__main__":
    print("""
      --------------------------------------------------
      [ACCRUAL AI] LOCAL HPC INFERENCE NODE ONLINE
      [TECH] CTYPES DOUBLE-POINTER BINDING + ASGI
      --------------------------------------------------
    """)
    uvicorn.run("accrual_inference_bridge:app_asgi", host="127.0.0.1", port=5000, reload=False)
