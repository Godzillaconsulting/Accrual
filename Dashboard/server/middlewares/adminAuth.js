...
const JWT_SECRET = process.env.JWT_SECRET || 'accrual_temp_secret_key_2026';
...
export const requireCM = (req, res, next) => {
    ...
    const allowed = user.role === 'admin' || user.role === 'cm' ||
        ['jareg', 'oscar', 'accrual_admin', 'judith'].includes(user.username?.toLowerCase() || req.admin.username?.toLowerCase());
